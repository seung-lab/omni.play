...
void tri_strip_to_obj( const float_type* points,
                       ...
                       const std::size_t strips_length )
{
    ...
    {
        std::cout << "v " << points[i] << ' ' << points[i+1] << ' ' << points[i+2] << '\n';
    }

    for ( std::size_t i = 3; i < points_length; i += 6 )
    {
        std::cout << "n " << points[i] << ' ' << points[i+1] << ' ' << points[i+2] << '\n';
    }
    ...
    for ( std::size_t i = 0; i < strips_length; i += 2 )
    {
        ...
        for ( index_type j = strips[i]; j < strips[i+1] - 1; ++j )
        {
            if ( even )
            {
                std::cout << "f " << indices[j] << ' ' << indices[j+1] << ' ' << indices[j+1] << '\n';
            }
            else
            {
                std::cout << "f " << indices[j+2] << ' ' << indices[j+1] << ' ' << indices[j] << '\n';
            }
            even = !even;
        }
    }
