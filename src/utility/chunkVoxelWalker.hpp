...
: public boost::iterator_facade<chunkVoxelWalker,
                    Vector3i const,
...
public:
    ...
    chunkVoxelWalker(const int x, const int y, const int z,
            const int chunkDim)
        : x(x), y(y), z(z)
        , chunkDim_(chunkDim)
    ...
    chunkVoxelWalker begin(){
        return chunkVoxelWalker(0,0,0, chunkDim_);
    }

    chunkVoxelWalker end(){ // one beyond the end
        return chunkVoxelWalker(chunkDim_,
                    chunkDim_,
                    chunkDim_,
                    chunkDim_);
    }
    ...
    bool operator <(const chunkVoxelWalker& rhs) const{
        if(chunkDim_ != rhs.chunkDim_){
            throw common::argException("different dimensions not allowed");
        }
        return (x < rhs.x &&
            y < rhs.y &&
            z < rhs.z);
...
private:
    ...
    bool operator !=(const chunkVoxelWalker&) const{
        throw common::argException("not implemented");
    }
