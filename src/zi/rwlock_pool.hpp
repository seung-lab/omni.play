...
public:
    ...
    std::size_t num_writers() const
    {
        return has_writer_ > 0 ? 1 : 0;
    }

    std::size_t num_clients() const
    {
        return num_readers() + num_writers();
    }
    ...
    inline bool timed_acquire_read( const T& client, const zi::mutex& m, int64_t ttl ) const
    {
        while ( has_writer_ || writer_waiting_ )
        {
    ...
    bool is_reader( const T& client ) const
    {
        return readers_.count( client ) > 0;
    }

    inline bool timed_acquire_write( const T& client, const zi::mutex& m, int64_t ttl ) const
    {
        while ( readers_.size() || has_writer_ )
        {
            writer_waiting_ = true;
...
private:
    ...
    inline rwlock_impl< ClientType >* find_or_insert_lock_nl( const LockType& lid ) const
    {
