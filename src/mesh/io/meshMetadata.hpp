...
public:
    ...
    bool IsHDF5() const
    {
        zi::guard g(lock_);
        if(!meshVersion_){
            throw common::ioException("meshes not yet built");
        }
...
private:
    ...
    bool load()
    {
        ...
        if(!file.open(QIODevice::ReadOnly)){
            throw common::ioException("error reading file", fnp_);
        }
        ...
        if(!in.atEnd()){
            throw common::ioException("corrupt file?", fnp_);
        }
    ...
    void store()
    {
        ...
        if (!file.open(QIODevice::WriteOnly)) {
            throw common::ioException("could not write file", fnp_);
        }
