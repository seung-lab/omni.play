...
std::unique_ptr<OmSegment> createSegment(std::vector<om::segment::Data> data,
  ...
  return std::make_unique<OmSegment>(data[segID], SEG_LIST_TYPE, VOLUME_SYSTEM);
}
void addEdge();
...
TEST(minCut, testNotSameRoot) {
  ...
  std::vector<om::segment::Data> data = prepareSegmentData(5);
  std::unique_ptr<OmSegment> rootSegment1(
      new OmSegment(data[1], SEG_LIST_TYPE, VOLUME_SYSTEM));
  std::unique_ptr<OmSegment> rootSegment2(
      new OmSegment(data[2], SEG_LIST_TYPE, VOLUME_SYSTEM));
