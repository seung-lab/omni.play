...
chunkCoord::chunkCoord() : Level(-1), Coordinate(Vector3i(-1, -1, -1)) {}
...
QString chunkCoord::getCoordsAsString() const {
  return QString("%1:%2,%3,%4").arg(Level).arg(Coordinate.x).arg(Coordinate.y)
      .arg(Coordinate.z);
}
...
chunkCoord chunkCoord::PrimaryChildCoord() const {
  ...
  return chunkCoord(Level - 1, Coordinate.x * 2, Coordinate.y * 2,
                    Coordinate.z * 2);
}
