...
class OmThreadPoolBatched {
 ...
 public:
  OmThreadPoolBatched() : taskVecSize_(1000) {}

  OmThreadPoolBatched(const int taskVecSize) : taskVecSize_(taskVecSize) {}
  ...
  inline void AddOrSpawnTasks(const ARG& t) {
    if (args_->size() >= taskVecSize_) {
      pool_.push_back(
          zi::run_fn(zi::bind(&OmThreadPoolBatched::worker, this, args_)));
