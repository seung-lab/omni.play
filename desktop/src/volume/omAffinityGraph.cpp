...
OmAffinityGraph::OmAffinityGraph(const OmID id) : OmManageableObject(id) {}
...
void OmAffinityGraph::ImportSingleChannel(const QString& hdf5fnp,
  ...
  channels_[aff] = om::make_shared<OmAffinityChannel>(aff);
...
OmAffinityChannel* OmAffinityGraph::GetChannel(
    const om::AffinityGraph aff) const {
  return channels_.find(aff)->second.get();
}

// use to just read data
om::shared_ptr<OmRawChunk<float> > OmAffinityGraph::RawChunk(
    const om::AffinityGraph aff, const om::chunkCoord& coord) {
  OmAffinityChannel* affChan = channels_[aff].get();
