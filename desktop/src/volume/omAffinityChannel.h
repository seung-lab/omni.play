...
class OmAffinityChannel : public OmChannel {
 ...
 public:
  OmAffinityChannel(const om::AffinityGraph aff) : OmChannel(), affinity_(aff) {
    if (!(aff == om::X_AFFINITY || aff == om::Y_AFFINITY ||
          aff == om::Z_AFFINITY)) {
  ...
  OmAffinityChannel(OmID id, const om::AffinityGraph aff)
      : OmChannel(id), affinity_(aff) {}

  ~OmAffinityChannel() {}

  om::AffinityGraph GetAffinity() const { return affinity_; }

  virtual QString GetDefaultHDF5DatasetName() { return "affGraphSingle"; }
