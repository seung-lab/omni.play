...
class OmDisplayInfo {
 public:
  ...
  void paint(const QString& str) { doPaint(str); }

  void paint(const int val, const QString& desc) {
    doPaint(QString::number(val), desc);
  }

  void paint(const double val, const QString& desc, const int numDecPlaces) {
    doPaint(QString::number(val, 'f', numDecPlaces), desc);
  }

  void paint(const float val, const QString& desc, const int numDecPlaces) {
    doPaint(QString::number(val, 'f', numDecPlaces), desc);
  }

  void paint(const int val1, const QString& desc1, const int val2,
             const QString& desc2) {
    const QString str =
        QString("%1 %2 %3 %4").arg(val1).arg(desc1).arg(val2).arg(desc2);
    doPaint(str);
  }
