...
class OmScreenPainter {
 ...
 private:
  ...
  inline bool amFilling() { return om::tool::FILL == toolMode_; }

  inline bool amErasing() { return om::tool::ERASE == toolMode_; }

  void drawFillTool(QPainter& painter) {
    painter.drawRoundedRect(QRect(mousePoint_.x, mousePoint_.y, 20, 20), 5, 5);
  }
  ...
  void showBrush(QPainter& painter) {
    const int brushDiamater =
        state_->getZoomScale() * state_->getBrushSize()->Diameter();
    const double offset = 0.5 * brushDiamater;

    painter.drawEllipse(QRectF(mousePoint_.x - offset, mousePoint_.y - offset,
                               brushDiamater, brushDiamater));
  }
  ...
  QString depthString() {
    ...
    QString depthStr =
        QString::number(globalDepth) + " - " + QString::number(dataDepth);
  ...
  void printTileCount(OmDisplayInfo& di) {
    ...
    if (tileCountIncomplete) {
      di.paint(tileCountIncomplete, "tiles incomplete of", tileCount, "tiles");
  ...
  void drawCursors(QPainter& painter) {
    ...
    painter.drawLine(halfWidth, 0, halfWidth, halfHeight - crosshairHoleSize);
    painter.drawLine(halfWidth, halfHeight + crosshairHoleSize, halfWidth,
                     fullHeight);

    painter.setPen(colors.second);

    painter.drawLine(0, halfHeight, halfWidth - crosshairHoleSize, halfHeight);
    painter.drawLine(fullWidth, halfHeight, halfWidth + crosshairHoleSize,
                     halfHeight);
  }
