...
Preferences3d::Preferences3d(QWidget *parent) : QWidget(parent) {
  ...
  antiAliasingCheckBox->setChecked(
      OmPreferences::GetBoolean(om::PREF_VIEW3D_ANTIALIASING_BOOL));
  highlightCheckBox->setChecked(
      OmPreferences::GetBoolean(om::PREF_VIEW3D_HIGHLIGHT_ENABLED_BOOL));
  transparentCheckBox->setChecked(
      OmPreferences::GetBoolean(om::PREF_VIEW3D_TRANSPARENT_UNSELECTED_BOOL));

  transparencySlider->setValue(floor(
      OmPreferences::GetFloat(om::PREF_VIEW3D_TRANSPARENT_ALPHA_FLT) * 50));
  transparencySliderLabel->setNum(
  ...
  const Vector3<float> &color2 =
      OmPreferences::GetVector3f(om::PREF_VIEW3D_HIGHLIGHT_COLOR_V3F);
  ...
  fovSlider->setValue(
      floor(OmPreferences::GetFloat(om::PREF_VIEW3D_CAMERA_FOV_FLT)));
  fovSliderLabel->setNum(
      floor(OmPreferences::GetFloat(om::PREF_VIEW3D_CAMERA_FOV_FLT)));

  nearplaneSlider->setValue(
      floor(OmPreferences::GetFloat(om::PREF_VIEW3D_CAMERA_NEAR_PLANE_FLT)));
  nearplaneSliderLabel->setNum(
      floor(OmPreferences::GetFloat(om::PREF_VIEW3D_CAMERA_NEAR_PLANE_FLT)));

  farplaneSlider->setValue(
      floor(OmPreferences::GetFloat(om::PREF_VIEW3D_CAMERA_FAR_PLANE_FLT)));
  farplaneSliderLabel->setNum(
      floor(OmPreferences::GetFloat(om::PREF_VIEW3D_CAMERA_FAR_PLANE_FLT)));

  axisCheckBox->setChecked(
      OmPreferences::GetBoolean(om::PREF_VIEW3D_SHOW_AXIS_BOOL));
  chunksCheckBox->setChecked(
      OmPreferences::GetBoolean(om::PREF_VIEW3D_SHOW_CHUNK_EXTENT_BOOL));
  infoCheckBox->setChecked(
      OmPreferences::GetBoolean(om::PREF_VIEW3D_SHOW_INFO_BOOL));
  focusCheckBox->setChecked(
      OmPreferences::GetBoolean(om::PREF_VIEW3D_SHOW_FOCUS_BOOL));

  styleComboBox->setCurrentIndex(
      OmPreferences::GetInteger(om::PREF_VIEW3D_FOCUS_STYLE_INT));

  om::connect(transparencySlider, SIGNAL(valueChanged(int)), this,
              SLOT(alphaSliderValueChanged(int)));
}
...
void Preferences3d::on_transparentCheckBox_stateChanged() {
  OmPreferences::SetBoolean(om::PREF_VIEW3D_TRANSPARENT_UNSELECTED_BOOL,
                            transparentCheckBox->isChecked());
}

void Preferences3d::on_fovSlider_valueChanged() {
  OmPreferences::SetFloat(om::PREF_VIEW3D_CAMERA_FOV_FLT,
                          (fovSlider->value() * 1.0));
}

void Preferences3d::on_nearplaneSlider_valueChanged() {
  OmPreferences::SetFloat(om::PREF_VIEW3D_CAMERA_NEAR_PLANE_FLT,
                          (nearplaneSlider->value()));
}

void Preferences3d::on_farplaneSlider_valueChanged() {
  OmPreferences::SetFloat(om::PREF_VIEW3D_CAMERA_FAR_PLANE_FLT,
                          (farplaneSlider->value() * 1000.0));
}

void Preferences3d::on_axisCheckBox_stateChanged() {
  OmPreferences::SetBoolean(om::PREF_VIEW3D_SHOW_AXIS_BOOL,
                            axisCheckBox->isChecked());
}

void Preferences3d::on_chunksCheckBox_stateChanged() {
  OmPreferences::SetBoolean(om::PREF_VIEW3D_SHOW_CHUNK_EXTENT_BOOL,
                            chunksCheckBox->isChecked());
}

void Preferences3d::on_infoCheckBox_stateChanged() {
  OmPreferences::SetBoolean(om::PREF_VIEW3D_SHOW_INFO_BOOL,
                            infoCheckBox->isChecked());
}

void Preferences3d::on_focusCheckBox_stateChanged() {
  OmPreferences::SetBoolean(om::PREF_VIEW3D_SHOW_FOCUS_BOOL,
                            focusCheckBox->isChecked());
}

void Preferences3d::on_styleComboBox_currentIndexChanged() {
  OmPreferences::SetInteger(om::PREF_VIEW3D_FOCUS_STYLE_INT,
                            styleComboBox->currentIndex());
}
...
void Preferences3d::alphaSliderValueChanged(int val) {
  ...
  OmPreferences::SetFloat(om::PREF_VIEW3D_TRANSPARENT_ALPHA_FLT,
                          (val / (50.0)));
}
