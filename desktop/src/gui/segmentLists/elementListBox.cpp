...
void ElementListBox::Reset() { instance().impl_->reset(); }
...
QWidget* ElementListBox::Widget() { return instance().impl_.get(); }

void ElementListBox::SetTitle(const QString& title) {
  instance().impl_->setTitle(title);
}

void ElementListBox::Delete() { instance().impl_.reset(); }

void ElementListBox::PopulateLists() { instance().impl_->PopulateLists(); }
