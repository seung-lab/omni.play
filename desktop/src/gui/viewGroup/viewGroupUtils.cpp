...
void om::gui::ViewGroupUtils::connectVisibilityChange(OmView2d* w,
                                                      QDockWidget* dock) {
  // used to let tile cache know when view2d visibilty changes
  om::connect(dock, SIGNAL(visibilityChanged(bool)), w,
              SLOT(dockVisibilityChanged(bool)));
}

void om::gui::ViewGroupUtils::setComplimentaryDockWidget(
    OmView2d* w, QDockWidget* dock, QDockWidget* complimentaryDock) {
