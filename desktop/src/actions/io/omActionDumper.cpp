...
void OmActionDumper::readAndDumpFile(const QFileInfo& fileInfo)
{
    ...
    in >> postfix;
    if("OMNI_LOG" != postfix){
        throw IoException("bad postfix", file.fileName());
    }

    if(!in.atEnd()){
        throw IoException("corrupt log file dectected", file.fileName());
    }
}
                                    ...
                                    const QString& fnp)
{
     ...
     case om::actions_::OmSegmentUncertainAction:
         doDumpFile<OmSegmentUncertainAction, OmSegmentUncertainActionImpl>(in, fnp);
         break;
     ...
     case om::actions_::OmProjectSaveAction:
         ...
         break;
     default:
         throw ArgException("unknown action", actionName);
     };
}
