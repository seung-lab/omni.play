...
public:
    ...
    QString classNameForLogFile() const {
        return "OmSegmentCutAction";
    }

private:
    template <typename T> friend class OmActionLoggerThread;
    friend class QDataStream &operator<<(QDataStream&, const OmSegmentCutActionImpl&);
    friend class QDataStream &operator>>(QDataStream&, OmSegmentCutActionImpl&);
    friend QTextStream& operator<<(QTextStream& out, const OmSegmentCutActionImpl&);
};
