...
template <typename DATA> class dataImpl : public dataInterface {
 ...
 public:
  ...
  void CopyInTile(const int sliceOffset, uchar const* const bits) {
    const int advance =
        (numElementsPerSlice_ * (sliceOffset % 128)) * sizeof(DATA);

    dataAccessor<DATA> dataWrapper(ptrToChunkData_);
    DATA* data = dataWrapper.Data();

    memcpy(data + advance, bits, numElementsPerSlice_ * sizeof(DATA));
  }
  ...
  void CopyInChunkData(OmDataWrapperPtr hdf5) {
    ...
    memcpy(data, dataHDF5, numElementsPerChunk_ * sizeof(DATA));
  }

  bool Compare(dataInterface const* const chunkInter) {
    dataImpl const* const other =
        reinterpret_cast<dataImpl const* const>(chunkInter);
  ...
  OmPooledTile<uint8_t>* ExtractDataSlice8bit(const om::common::ViewType plane,
                                              const int depth) {
    dataAccessor<DATA> dataWrapper(ptrToChunkData_);
    DATA* data = dataWrapper.Data();
