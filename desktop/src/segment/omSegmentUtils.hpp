...
class OmSegmentUtils {
 public:
  om::shared_ptr<std::set<OmSegment*> > static GetAllChildrenSegments(
      OmSegments* segments, const OmSegIDsSet& set) {
  ...
  template <class B>
  static void GetAllChildrenSegments(OmSegments* segments, const OmSegID segID,
                                     B& ret) {
    OmSegmentIterator iter(segments);
  ...
  om::shared_ptr<std::deque<std::string> > static GetChildrenInfo(
  ...
  static QColor SetSegColor(const SegmentDataWrapper& sdw,
    ...
    const OmColor c = { static_cast<uint8_t>(color.red()),
