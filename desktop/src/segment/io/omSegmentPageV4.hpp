...
class OmSegmentPageV4 {
 ...
 public:
  ...
  OmSegmentDataV4* Create() {
    data_ = OmMemMappedFileQTNew<OmSegmentDataV4>::CreateNumElements(fnp_,
                                                                     pageSize_);
    return data_->GetPtr();
  }
 ...
 private:
  std::string path() { return pathQStr().toStdString(); }

  QString pathQStr() const {
    const std::string fname =
        str(boost::format("segment_page%1%.data.ver4") % pageNum_);
