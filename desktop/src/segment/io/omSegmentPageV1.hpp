...
class OmSegmentPageV1 {
 ...
 public:
  OmSegmentPageV1(OmSegmentation* segmentation,
                  const om::common::PageNum pageNum, const uint32_t pageSize)
  ...
  std::shared_ptr<OmSegmentDataV3> Read() {
    ...
    OmDataArchiveSegment::ArchiveRead(getOldHDF5path(), oldSegmentData,
                                      pageSize_);
 ...
 private:
  OmDataPath getOldHDF5path() {
    return OmDataPaths::getSegmentPagePath(segmentation_->GetID(), pageNum_);
  }
};
