...
class OmSegmentPageConverter {
 public:
  std::shared_ptr<OmSegmentDataV4> static ConvertPageV3toV4(
      std::shared_ptr<OmSegmentDataV3> dataV3, const uint32_t pageSize) {
    OmSegmentDataV3* rawV3 = dataV3.get();

    std::shared_ptr<OmSegmentDataV4> ret =
        om::mem::Malloc<OmSegmentDataV4>::NumElements(pageSize,
  ...
  std::shared_ptr<uint8_t> static ConvertPageV3toV4ListType(
      std::shared_ptr<OmSegmentDataV3> dataV3, const uint32_t pageSize) {
    OmSegmentDataV3* rawV3 = dataV3.get();

    std::shared_ptr<uint8_t> ret = om::mem::Malloc<uint8_t>::NumElements(
        pageSize, om::mem::ZeroFill::ZERO);
    uint8_t* raw = ret.get();
