...
void OmSegmentGraph::RefreshGUIlists() { segmentListsLL_->RefreshGUIlists(); }
...
void OmSegmentGraph::SetGlobalThreshold(OmMST* mst) {
  OmSegmentGraphInitialLoad loader(forest_.get(), validGroupNum_,
                                   segmentListsLL_, segmentPages_,
                                   children_.get());
...
void OmSegmentGraph::ResetGlobalThreshold(OmMST* mst) {
  ...
  for (uint32_t i = 0; i < mst->NumEdges(); ++i) {
    ...
    if (edges[i].threshold >= stopThreshold ||
      ...
      if (joinInternal(edges[i].node2ID, edges[i].node1ID, edges[i].threshold,
                       i)) {
        edges[i].wasJoined = 1;
      } else {
        edges[i].userSplit = 1;
...
void OmSegmentGraph::UpdateSizeListsFromJoin(OmSegment* parent,
                                             OmSegment* child) {
  OmSegment* root = mCache->FindRoot(parent);
  segmentListsLL_->UpdateSizeListsFromJoin(root, child);
}

void OmSegmentGraph::UpdateSizeListsFromSplit(OmSegment* parent,
                                              OmSegment* child) {
  OmSegment* root = mCache->FindRoot(parent);

  const SizeAndNumPieces childInfo = computeSegmentSizeWithChildren(child);

  segmentListsLL_->UpdateSizeListsFromSplit(root, child, childInfo);
}

SizeAndNumPieces OmSegmentGraph::computeSegmentSizeWithChildren(
    OmSegment* inSeg) {
  int64_t numVoxels = 0;
  int32_t numPieces = 0;
