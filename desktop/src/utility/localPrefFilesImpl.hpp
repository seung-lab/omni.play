...
// number
    ...
    inline T readSettingNumber(const QString& setting)
    {
        ...
        if(0 == lines.size()){
            throw om::IoException("invalid preference found", setting);
        }
    ...
    inline void writeSettingNumber(const QString& setting, const T value)
    {
        ...
        if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
            throw om::IoException("could not write file", fnp);
        }
...
// QString
    inline QString readSettingQString(const QString& setting)
    {
        ...
        if(1 != lines.size()){
            throw om::IoException("no preference found", setting);
        }
...
private:
    ...
    inline void setupPrefFolder()
    {
        ...
        if(QDir::home().mkdir(omniFolderName)){
            ...
            throw om::IoException("could not create folder", omniFolderPath);
        }
    }
    ...
    inline QStringList readFile(const QString& setting)
    {
        ...
        if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
            throw om::IoException("could not read file", fnp);
        }
