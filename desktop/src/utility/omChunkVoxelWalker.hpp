...
class OmChunkVoxelWalker : public boost::iterator_facade<
 ...
 public:
  ...
  OmChunkVoxelWalker(const int x, const int y, const int z, const int chunkDim)
      : x(x), y(y), z(z), chunkDim_(chunkDim), curPos_(Vector3i(x, y, z)) {}

  OmChunkVoxelWalker begin() { return OmChunkVoxelWalker(0, 0, 0, chunkDim_); }

  OmChunkVoxelWalker end() {  // one beyond the end
    return OmChunkVoxelWalker(chunkDim_, chunkDim_, chunkDim_, chunkDim_);
  }
