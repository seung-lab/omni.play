...
class DataWrapperContainer {
 ...
 public:
  ...
  DataWrapperContainer(const om::common::ObjectType obj_type,
                       const om::common::ID obj_id) {
    switch (obj_type) {
  ...
  bool isValidContainer() const { return sdw_ || cdw_ || adw_; }

  bool isSegmentation() const { return sdw_; }

  bool isChannel() const { return cdw_; }

  bool isAffinityGraph() const { return adw_; }
  ...
  SegmentationDataWrapper GetSDW() const { return *sdw_; }

  ChannelDataWrapper getChannelDataWrapper() const { return *cdw_; }

  AffinityGraphDataWrapper GetADW() const { return *adw_; }
};
