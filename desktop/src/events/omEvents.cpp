...
void OmEvents::Redraw2d() { POST(new OmViewEvent(OmViewEvent::REDRAW)); }

void OmEvents::Redraw2dBlocking() {
  POST(new OmViewEvent(OmViewEvent::REDRAW_BLOCKING));
}

void OmEvents::Redraw3d() { POST(new OmView3dEvent(OmView3dEvent::REDRAW)); }
...
void OmEvents::SegmentModified(std::shared_ptr<OmSelectSegmentsParams> params) {
...
void OmEvents::RefreshMSTthreshold() { POST(new OmRefreshMSTthresholdEvent()); }
...
void OmEvents::AnnotationEvent() {
  POST(new om::events::annotationEvent(
      om::events::annotationEvent::ANNOTATION_OBJECT_MODIFICATION));
}
