...
class OmEventManagerImpl : public QObject {
 ...
 public:
  ...
  void AddListener(OmEventClass eventClass, OmEventListener* listener) {
    zi::guard g(lock_);
    classListeners_[eventClass].insert(listener);
  }

  void RemoveListener(OmEventClass eventClass, OmEventListener* listener) {
    zi::guard g(lock_);
 ...
 private:
  ...
  bool event(QEvent* event) {
    ...
    FOR_EACH(it, listeners) { omEvent->Dispatch(*it); }
