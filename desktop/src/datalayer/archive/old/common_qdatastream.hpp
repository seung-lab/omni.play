...
template <class T>
QDataStream& operator<<(QDataStream& out, const AxisAlignedBoundingBox<T>& d) {
...
template <class T>
    QDataStream& operator>>(QDataStream& in, AxisAlignedBoundingBox<T>& d) {
...
template <class T>
QDataStream& operator<<(QDataStream& out, const Matrix4<T>& m) {
  for (int i = 0; i < 16; ++i) {
    out << m.array[i];
  }
  return out;
