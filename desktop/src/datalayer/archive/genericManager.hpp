...
public:
    template <class T>
    static void Save(YAML::Emitter& out, const om::common::GenericManager<T>& gm)
    {
    ...
    template <class T>
    static void Load(const YAML::Node& in, om::common::GenericManager<T>& gm, void(*parse)(const YAML::Node&, T*) = NULL)
    {
        in["size"] >> gm.size_;
