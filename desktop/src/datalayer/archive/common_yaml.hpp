...
inline void operator>>(const YAML::Node& in, int& p) { p = in.as<int>(); }

inline void operator>>(const YAML::Node& in, uint32_t& p) {
  p = in.as<uint32_t>();
}

inline void operator>>(const YAML::Node& in, double& p) { p = in.as<double>(); }

inline void operator>>(const YAML::Node& in, float& p) { p = in.as<float>(); }

inline void operator>>(const YAML::Node& in, bool& p) { p = in.as<bool>(); }
