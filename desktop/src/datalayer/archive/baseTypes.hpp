...
template<class T>
void operator>>(const YAMLold::Node& in, Vector3<T>& p)
{
    in[0] >> p.x;
    in[1] >> p.y;
    in[2] >> p.z;
...
template<class T>
void operator>>(const YAMLold::Node& in, Matrix4<T>& p)
{
    T f[16];
    for(int i = 0; i < 16; i++)
    {
...
template<class T>
YAMLold::Emitter &operator<<(YAMLold::Emitter& out, const vmml::AxisAlignedBoundingBox<T>& b)
{
...
template<class T>
YAMLold::Emitter &operator<<(YAMLold::Emitter& out, const boost::unordered_set<T>& s)
{
    out << YAMLold::Flow << YAMLold::BeginSeq;
    FOR_EACH(it, s)
    {
...
template<class T>
void operator>>(const YAMLold::Node& in, boost::unordered_set<T>& s)
{
    FOR_EACH(it, in)
    {
        T item;
...
template<class T>
void operator>>(const YAMLold::Node& in, std::set<T>& s)
{
    FOR_EACH(it, in)
    {
        T item;
