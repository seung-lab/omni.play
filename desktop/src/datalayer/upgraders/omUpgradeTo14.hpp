...
class OmUpgradeTo14 {
 ...
 private:
  ...
  template <typename T> void convertVolume(T& vol) {
    allocate(vol);
    copyData(vol);
  }

  template <typename T> void allocate(T& vol) {
  ...
  template <typename T> void copyData(T& vol) {
  ...
  template <typename T> void copyChunk(T& vol, const om::chunkCoord& coord) {
