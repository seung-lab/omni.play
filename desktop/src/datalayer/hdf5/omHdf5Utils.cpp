...
void OmHdf5Utils::group_create(const int fileId, const char* path) {
  ...
  if (group_id < 0) {
    throw OmIoException("Could not create HDF5 group \"" + std::string(path) +
                        "\"");
  }
...
bool OmHdf5Utils::dataset_exists(const int fileId, const char* path) {
  ...
  if (ret < 0) {
    throw OmIoException("Could not close HDF5 dataset " + std::string(path));
  }
...
void OmHdf5Utils::dataset_delete(const int fileId, const char* path) {
  ...
  herr_t err = H5Gunlink(fileId, path);
  if (err < 0) {
    throw OmIoException("Could not unlink HDF5 dataset " + std::string(path));
  }
}
...
OmDataWrapperPtr OmHdf5Utils::getDataWrapper(
    void* dataset, const int dstype, const om::OmDataAllocType allocType) {
