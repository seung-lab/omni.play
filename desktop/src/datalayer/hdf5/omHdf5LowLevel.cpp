...
OmHdf5LowLevel::OmHdf5LowLevel(const int id)
    : fileId(id)
{}

void OmHdf5LowLevel::setPath(const OmDataPath & p){
    path = p;
}

const char* OmHdf5LowLevel::getPath(){
...
OmDataWrapperPtr OmHdf5LowLevel::readDataset(int *size)
{
    ...
    if (dataspace_id < 0) {
        throw IoException("Could not get HDF5 dataspace.");
    }
    ...
    if (NULL == dataset_data) {
        throw IoException("Could not allocate memory to read HDF5 dataset.");
    }
    ...
    if (status < 0) {
        throw IoException("Could not close HDF5 dataspace.");
    }
...
void OmHdf5LowLevel::allocateDataset(int size, OmDataWrapperPtr data)
{
    ...
    if (dataspace_id < 0) {
        throw IoException("Could not create HDF5 dataspace.");
    }
    ...
    if (dataset_id < 0) {
        throw IoException("Could not create HDF5 dataset " + std::string(getPath()));
    }
    ...
    if (NULL != data->getVoidPtr()) {
        status = H5Dwrite(dataset_id, data->getHdf5FileType(),
                          ...
                          data->getVoidPtr());
        if (status < 0)
            throw IoException("Could not copy data into HDF5 dataset.");
    }
...
Vector3i  OmHdf5LowLevel::getChunkedDatasetDims(const om::common::AffinityGraph affin)
{
    ...
    hid_t dataset_id = H5Dopen2(fileId, getPath(), H5P_DEFAULT);
    if (dataset_id < 0) {
        throw IoException("Could not open HDF5 dataset " + std::string(getPath()));
    }
    ...
    if (rank < 0) {
        throw IoException("Could not determine rank of HDF5 dataspace.");
    }

    if (4 == rank && aff == om::common::NO_AFFINITY){
        aff = om::common::X_AFFINITY;
        //throw IoException("HDF5 dataspace rank 4 but doesn't have a set affinity.");
    }
    ...
    if(4 == rank) {
        ...
        rank = H5Sget_simple_extent_dims(dataspace_id,
                                         ...
                                         maxdims4.array);
        if (rank < 0) {
            throw IoException("Could not determine dimensions of HDF5 dataspace.");
        }
                                            ...
                                            const om::common::DataType type)
{
    ...
    if (plist_id < 0) {
        throw IoException("Could not create HDF5 property list.");
    }
    ...
    if (ret < 0) {
        throw IoException("Could not set HDF5 chunk size.");
    }
    hid_t whenAlloc = H5Pset_alloc_time( plist_id, H5D_ALLOC_TIME_EARLY);
    if (whenAlloc < 0) {
        throw IoException("Could not set HDF5 alloc time.");
    }
    ...
    if (ret < 0) {
        throw IoException("Could not close HDF5 property list.");
    }
...
void OmHdf5LowLevel::writeChunk(const om::coords::DataBbox& extent, OmDataWrapperPtr data)
{
    ...
    herr_t ret = H5Sselect_hyperslab(dataspace_id,
                                     ...
                                     block_flipped.array);
    if (ret < 0)
        throw IoException("Could not select HDF5 hyperslab.");

    //Creates a new simple dataspace and opens it for access.
    hid_t mem_dataspace_id = H5Screate_simple(3, block.array, block.array);
    if (mem_dataspace_id < 0)
        throw IoException("Could not create scratch HDF5 dataspace to read data into.");
    ...
    ret = H5Dwrite(dataset_id, type,
                   mem_dataspace_id,
                   dataspace_id,
                   H5P_DEFAULT,
                   rawData);
    if (ret < 0) {
        throw IoException("Could not read HDF5 dataset " + std::string(getPath()));
    }
    ...
    if (ret < 0)
        throw IoException("Could not close HDF5 scratch dataspace.");
                                           ...
                                           const om::common::AffinityGraph aff)
{
    ...
    if(4 == rank && om::common::NO_AFFINITY != aff) {		// Load a side of the affinity graph.
        ...
        ret = H5Sselect_hyperslab(dataspace_id,
                                  H5S_SELECT_SET,
                                  start_flipped.array,
                                  stride.array,
                                  count.array,
        ...
        ret = H5Dread(dataset_id,
                      data->getHdf5MemoryType(),
                      mem_dataspace_id,
                      dataspace_id,
                      H5P_DEFAULT,
                      imageData);
        if (ret < 0) {
            std::cout << "ERROR: extexts were "
                      << extent << "\n";
            throw IoException("Error while reading HDF5 dataset \""
                                + std::string(getPath()) + "\"");
        }
