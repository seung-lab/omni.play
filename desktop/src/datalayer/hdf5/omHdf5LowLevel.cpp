...
Vector3i OmHdf5LowLevel::getChunkedDatasetDims(const om::AffinityGraph affin) {
  ...
  if (4 == rank && aff == om::NO_AFFINITY) {
    aff = om::X_AFFINITY;
    //throw OmIoException("HDF5 dataspace rank 4 but doesn't have a set
    //affinity.");
  }
...
void OmHdf5LowLevel::writeChunk(const om::dataBbox& extent,
  ...
  if (ret < 0) throw OmIoException("Could not select HDF5 hyperslab.");
  ...
  if (ret < 0) throw OmIoException("Could not close HDF5 scratch dataspace.");

  ret = H5Sclose(dataspace_id);
  if (ret < 0) throw OmIoException("Could not close HDF5 dataspace.");

  //Closes the specified dataset.
  //herr_t H5Dclose(hid_t dataset_id  )
  ret = H5Dclose(dataset_id);
  if (ret < 0) throw OmIoException("Could not close HDF5 dataset.");
}
...
OmDataWrapperPtr OmHdf5LowLevel::readChunk(const om::dataBbox& extent,
  ...
  hid_t dataspace_id = H5Dget_space(dataset_id);
  if (dataspace_id < 0) throw OmIoException("Could not get HDF5 dataspace.");
  ...
  if (4 == rank &&
      om::NO_AFFINITY != aff) {  // Load a side of the affinity graph.
    //Selects a hyperslab region to add to the current selected region.
...
Vector3i OmHdf5LowLevel::getDatasetDims() {
  ...
  //Releases and terminates access to a dataspace.
  status = H5Sclose(dataspace_id);
  if (status < 0) throw OmIoException("Could not close HDF5 dataspace.");

  //Closes the specified dataset.
  status = H5Dclose(dataset_id);
  if (status < 0) throw OmIoException("Could not close HDF5 dataset.");
