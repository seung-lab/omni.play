...
void OmHdf5LowLevel::allocateDataset(int size, OmDataWrapperPtr data)
{
    ...
    if (dataset_id < 0) {
        throw om::IoException("Could not create HDF5 dataset " + std::string(getPath()));
    }
...
Vector3i  OmHdf5LowLevel::getChunkedDatasetDims(const om::common::AffinityGraph affin)
{
    ...
    hid_t dataset_id = H5Dopen2(fileId, getPath(), H5P_DEFAULT);
    if (dataset_id < 0) {
        throw om::IoException("Could not open HDF5 dataset " + std::string(getPath()));
    }
    ...
    if (4 == rank && aff == om::common::AffinityGraph::NO_AFFINITY){
        ...
        //throw om::IoException("HDF5 dataspace rank 4 but doesn't have a set affinity.");
    }
...
void OmHdf5LowLevel::writeChunk(const om::dataBbox& extent, OmDataWrapperPtr data)
{
    ...
    if (mem_dataspace_id < 0)
        throw om::IoException("Could not create scratch HDF5 dataspace to read data into.");
    ...
    ret = H5Dwrite(dataset_id, type,
                   ...
                   rawData);
    if (ret < 0) {
        throw om::IoException("Could not read HDF5 dataset " + std::string(getPath()));
    }
    ...
    if (ret < 0)
        throw om::IoException("Could not close HDF5 scratch dataspace.");
                                           ...
                                           const om::common::AffinityGraph aff)
{
    ...
    if(4 == rank && om::common::AffinityGraph::NO_AFFINITY != aff) {		// Load a side of the affinity graph.
        //Selects a hyperslab region to add to the current selected region.
