...
#define MEX_TYPE_SPEC_CLASS(_DTYPE_, _MXTYPE_, _MXCFN_) \
  ...
  bool MexType<_DTYPE_>::CHECK(const mxArray *pa) { return _MXCFN_(pa); }
...
#define MEXPRINTF(...)                                                       \
  do {                                                                       \
    mexPrintf("%s (%0.3f): ", S_MEX_NAME.c_str(), zTimer.elapsed<double>()); \
...
#define DECLARE_MEX_INT(_VAR_, _PRHS_) DECLARE_MEX_SCALAR(_VAR_, _PRHS_, int)
...
std::string getMexString(const mxArray *prhs) {
  ...
  ret.resize(len);
  for (int i = 0; i < len; i++) ret[i] = (unsigned char)(chars[i] % 256);
  return ret;
}
