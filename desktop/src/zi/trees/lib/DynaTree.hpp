...
template <typename _T> DynamicTree<_T>::DynamicTree(const value_t &v) {
  impl_ = new Impl(v);
#ifdef DYNAMIC_TREE_TEST
  zRefCnt++;
#endif
...
template <typename _T> DynamicTree<_T>::DynamicTree(const DynamicTree &t) {
  impl_ = t.impl_;
  incRef();
}

template <typename _T> DynamicTree<_T>::DynamicTree(Impl *impl) {
  impl_ = impl;
  incRef();
}

template <typename _T> DynamicTree<_T>::~DynamicTree() { decRef(); }

template <typename _T> void DynamicTree<_T>::decRef() {
  if (impl_ != NULL) {
...
template <typename _T> void DynamicTree<_T>::incRef() {
  if (impl_ != NULL) {
    ++impl_->refCount_;
  }
}
...
template <typename _T> void DynamicTree<_T>::cut() {
  if (impl_ != NULL) impl_->cut();
}

template <typename _T> void DynamicTree<_T>::join(const DynamicTree &x) {
  assert(x.impl_ != NULL);
  assert(impl_ != NULL);
  impl_->join(x.impl_);
}

template <typename _T> DynamicTree<_T> DynamicTree<_T>::findRoot() {
  if (impl_ != NULL)
...
template <typename _T> bool DynamicTree<_T>::isRoot() {
  if (impl_ != NULL)
    return impl_->isRoot();
  else
    return true;
...
template <typename _T> class DynamicTree<_T>::Impl {
 public:
...
template <typename T> class DNMTREE {
 public:
...
template <typename T> DNMTREE<T> *DNMTREE<T>::makeTree(const T &key) {
  return new DNMTREE(key);
}

template <typename T> DNMTREE<T> *DNMTREE<T>::cut() {
  access();
  if (left_ != NULL) {
...
template <typename T> void DNMTREE<T>::join(DNMTREE *w) {
  access();
  assert(left_ == NULL);
...
template <typename T> DNMTREE<T> *DNMTREE<T>::findRoot() {
  access();
  DNMTREE *v = this;
  while (v->left_ != NULL) {
    v = v->left_;
...
template <typename T> void DNMTREE<T>::access() {
  splay();
  if (right_) {
    right_->pathParent_ = this;
    right_->parent_ = NULL;
...
template <typename T> void DNMTREE<T>::rotateLeft() {
  assert(right_ != NULL);
...
template <typename T> void DNMTREE<T>::rotateRight() {
  assert(left_ != NULL);
...
template <typename T> void DNMTREE<T>::splay() {
  DNMTREE *g;
  while (parent_ != NULL) {
    g = parent_->parent_;
    if (this == parent_->left_) {
...
template <typename T> void DNMTREE<T>::printTree(int d) {
...
template <typename T> DNMTREE<T> *DNMTREE<T>::findMin() {
  DNMTREE *x = this;
...
template <typename T> DNMTREE<T> *DNMTREE<T>::findMax() {
  DNMTREE *x = this;
