...
class OmDataForMeshLoad {
 public:
  ...
  inline void HasData(const bool val) { hasData_ = val; }

  uint64_t NumBytes() const {
    return vertexIndexNumBytes_ + vertexDataNumBytes_ + stripDataNumBytes_ +
           trianDataNumBytes_;
  }
  ...
  inline uint32_t VertexIndexCount() const { return vertexIndexCount_; }
  ...
  inline uint32_t StripDataCount() const { return stripDataCount_; }
  ...
  inline uint32_t VertexDataCount() const { return vertexDataCount_; }
  ...
  inline uint32_t TrianDataCount() const { return trianDataCount_; }
