...
class OmThreadedTileCache : public OmCacheBase {
 ...
 public:
  OmThreadedTileCache(const std::string& name, const int bytesPerTile)
      : OmCacheBase(name, om::common::CacheGroup::TILE_CACHE),
        bytesPerTile_(bytesPerTile),
        threadPool_(OmTileCache::ThreadPool()) {
    freshness_.set(0);

    OmCacheManager::AddCache(om::common::CacheGroup::TILE_CACHE, this);
  }
  ...
  virtual void Get(ptr_t& ptr, const key_t& key,
    ...
    threadPool_.QueueUpIfKeyNotPresent(
        key, &OmThreadedTileCache::handleCacheMissThread, this);
  }
  ...
  void BlockingCreate(ptr_t& ptr, const key_t& key) { ptr = loadItem(key); }
  ...
  void Remove(const key_t& key) { cache_.erase(key); }
  ...
  void ClearFetchQueue() { threadPool_.clear(); }
  ...
  int64_t GetCacheSize() const { return cache_.size() * bytesPerTile_; }
