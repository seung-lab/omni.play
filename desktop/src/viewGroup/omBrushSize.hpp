...
Q_SIGNALS:
 ...
 public:
  OmBrushSize() : diameter_(8) {}

  virtual ~OmBrushSize() {}

  inline int Diameter() const { return diameter_; }

  void SetDiameter(const int size) { setDiameter(size); }

  void IncreaseSize() { setDiameter(getNextSizeUp()); }

  void IncreaseSize(const int inc) { setDiameter(diameter_ + inc); }

  void DecreaseSize() { setDiameter(getNextSizeDown()); }
 ...
 private:
  ...
  void setPtsInCircle() { ptsInBrush_ = circlePts_.GetPtsInCircle(diameter_); }
