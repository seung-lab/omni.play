  ...
  switch (volType_) {
    case server::volType::CHANNEL: {
      const std::string chanName =
          str(boost::format("%1%/channels/channel1/%2%/volume.float.raw") %
              uri_ % mipLevel);
      ...
      mapData mapped(chanName,
                     server::dataType::FLOAT);  // Ignore dataType for now.
    ...
    case server::volType::SEGMENTATION: {
      const std::string segName =
          str(boost::format(
                  "%1%/segmentations/segmentation1/%2%/volume.uint32_t.raw") %
              uri_ % mipLevel);
      ...
      mapData mappedSeg(segName,
                        server::dataType::UINT32);  // Ignore dataType for now.
...
volume::volume(std::string uri, coords::globalBbox bounds, Vector3i resolution,
...
volume::volume(std::string uri, common::objectType type) : uri_(uri) {
  YAML::Node n;
...
boost::optional<segments::data> volume::GetSegmentData(int32_t segId) const {
  ...
  const std::string fname =
      str(boost::format("%1%/users/_default/segmentations/segmentation1/"
                        "segments/segment_page%2%.data.ver4") %
          uri_ % pageNum);
