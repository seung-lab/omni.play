...
class volume {
 ...
 public:
  ...
  volume(std::string uri, coords::globalBbox bounds, Vector3i resolution,
         server::dataType::type dataType, server::volType::type volType,
         Vector3i chunkDims);
  ...
  inline const coords::globalBbox& Bounds() const { return bounds_; }

  inline const Vector3i& Resolution() const { return resolution_; }

  inline server::dataType::type DataType() const { return dataType_; }

  inline server::volType::type VolumeType() const { return volType_; }

  inline const Vector3i& ChunkDims() const { return chunkDims_; }
...
inline std::ostream& operator<<(std::ostream& out, const volume& vol) {
  out << "{" << std::endl << "\turi:\t" << vol.uri_ << std::endl
      << "\tbounds:\t" << vol.bounds_ << std::endl << "\tresolution:\t"
      << vol.resolution_ << std::endl << "\tdataType:\t" << vol.dataType_
      << std::endl << "\tvolType:\t" << vol.volType_ << std::endl
