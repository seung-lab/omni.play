...
class unchunk  //: public stage
 ...
 public:
  unchunk(const coords::volumeSystem& vs)
      : vs_(vs), dims_(vs_.GetDataDimensions()) {}
  ...
  data<T> operator()(const datalayer::memMappedFile<T>& in) const {
    ...
    boost::shared_ptr<std::deque<coords::chunk> > chunks =
        vs_.GetMipChunkCoords(0);
    FOR_EACH(iter, *chunks) {
      ...
      for (int z = 0; z < dims.z; ++z) {
        for (int y = 0; y < dims.y; ++y) {
          ...
          std::copy(&chunkPtr[off], &chunkPtr[off + dims.x],
                    &outPtr[target_offset(d)]);
        }
      }
    }
    return out;
