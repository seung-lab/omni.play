...
class getSegId : public boost::static_visitor<uint32_t> {
 ...
 public:
  ...
  uint32_t operator()(const datalayer::memMappedFile<T>& in) const {
    uint64_t offset =
        coord_.toChunk().chunkPtrOffset(coord_.volume(), sizeof(T));
...
class getSegIds : public stage {
 ...
 public:
  getSegIds(coords::data coord, int radius, common::viewType view,
            coords::dataBbox bounds)
      : coord_(coord), radius_(radius), view_(view), bounds_(bounds) {}
