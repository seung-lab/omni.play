...
template <typename T> class rawChunkSlicer {
 ...
 public:
  ...
  boost::shared_ptr<T> GetCopyOfTile(const common::viewType viewType,
                                     const int offsetNumTiles) {
    boost::shared_ptr<T> tilePtr = utility::smartPtr<T>::MallocNumElements(
 ...
 private:
  void sliceTile(const common::viewType viewType, const int offsetNumTiles,
                 T* tile) {
    switch (viewType) {
