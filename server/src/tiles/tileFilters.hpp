...
template <typename T> class filters {
 ...
 public:
  ...
  void Brightness(boost::shared_ptr<T> tile, const T absMax,
                  const int32_t shift) {
    zi::transform(tile, tile + elementsPerTile_, tile,
                  ChangeBrightness<T>(absMax, shift));
  }

  void Contrast(boost::shared_ptr<T> tile, const T absMax,
  ...
  boost::shared_ptr<C> recast(boost::shared_ptr<T> oldTile) const {
    boost::shared_ptr<C> ret = utility::smartPtr<C>::MallocNumElements(
        elementsPerTile_, common::DONT_ZERO_FILL);

    std::copy(oldTile.get(), oldTile.get() + elementsPerTile_, ret.get());
  ...
  boost::shared_ptr<C> rescaleAndCast(boost::shared_ptr<T> oldTile,
    ...
    zi::transform(oldTile.get(), oldTile.get() + elementsPerTile_, ret.get(),
