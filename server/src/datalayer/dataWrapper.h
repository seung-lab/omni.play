...
template <class T> class dataWrapper : public dataWrapperBase {
 public:
  typedef boost::shared_ptr<dataWrapper<T> > ptr_type;

  explicit dataWrapper(boost::shared_ptr<T> sptr) : ptr_(sptr) {}

  static dataWrapperPtr produceNull() { return ptr_type(new dataWrapper()); }
  ;
  ...
  volume::dataType getVolDataType() { return volDataTypeImpl<T>::getType(); }
 ...
 private:
  ...
  explicit dataWrapper() : ptr_() {}

  dataWrapper(T* ptr, const dataAllocType d) : ptr_(wrapRawPtr(ptr, d)) {}
...
template <typename T> static dataWrapperPtr Wrap(boost::shared_ptr<T> sptr) {
  return boost::make_shared<dataWrapper<T> >(sptr);
}

template <typename T> boost::shared_ptr<T> UnWrap(const dataWrapperPtr wrap) {
  dataWrapper<T>* dataPtrReint = reinterpret_cast<dataWrapper<T>*>(wrap.get());
