...
template <typename T> class memMappedFile : public IOnDiskFile<T> {
 public:
  static boost::shared_ptr<memMappedFile<T> > CreateNumElements(
  ...
  static boost::shared_ptr<memMappedFile<T> > CreateNumBytes(
  ...
  static boost::shared_ptr<memMappedFile<T> > CreateFromData(
      const std::string& fnp, boost::shared_ptr<T> d,
