...
template <typename T> boost::shared_ptr<T> readAll(std::string* fnp) {
...
template <typename T> void writeStrings(std::string& file, const T& strings) {
  std::fstream out(file.c_str(), std::fstream::out);

  FOR_EACH(iter, strings) { out << *iter << "\n"; }
}
