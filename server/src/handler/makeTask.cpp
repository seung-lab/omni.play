...
template <typename T>
void connectedSets(
    const coords::globalBbox& bounds, const volume::volume& vol,
    const T& allowed,
    std::vector<boost::shared_ptr<std::set<uint32_t> > >& results) {
  uint32_t max_seg_id = 0;
  FOR_EACH(it, allowed) { max_seg_id = std::max(max_seg_id, *it); }
  ...
  boost::unordered_map<uint32_t, boost::shared_ptr<std::set<uint32_t> > >
      newSeedSets;
  ...
  FOR_EACH(it, newSeedSets) { results.push_back(it->second); }
}
...
typedef std::pair<boost::shared_ptr<SegBundle>, boost::shared_ptr<SegBundle> >
    BundlePair;

struct Overlap {
  Overlap(const volume::volume& pre, const std::set<int32_t>& preSegs,
          const volume::volume& post)
      : pre_(pre), post_(post) {
    bounds_ = pre_.Bounds();
  ...
  std::vector<BundlePair> GetBundles() {
    ...
    FOR_EACH(seg, mappingCounts_) { postIds.insert(seg->first); }
    ...
    FOR_EACH(postBundle, postBundles) {
      ...
      FOR_EACH(seg, **postBundle) {
        preBundle->insert(postPreCorrespondence_[*seg].begin(),
                          postPreCorrespondence_[*seg].end());
      }
  ...
  std::map<int32_t, int32_t> MakeSeed(const SegBundle& bundle) {
    ...
    FOR_EACH(seg, bundle) {
      if (((double) mappingCounts_[*seg]) / ((double) sizes_[*seg]) >=
          FALSE_OBJ_RATIO_THR) {
        ret[*seg] = sizes_[*seg];
      }
      if (sizes_[*seg] > largestSize) {
        largest = *seg;
        largestSize = sizes_[*seg];
      }
    }
...
void get_seeds(std::vector<std::map<int32_t, int32_t> >& seeds,
  ...
  std::cout << "Getting Seeds" << std::endl << taskVolume.Uri() << std::endl
  ...
  FOR_EACH(bundle, bundles) {
    ...
    FOR_EACH(seg, *bundle->first) { std::cout << *seg << ","; }
    std::cout << std::endl;

    std::cout << "Post Segments: ";
    FOR_EACH(seg, *bundle->second) { std::cout << *seg << ","; }
    std::cout << std::endl;

    if (!overlap.Leaves(*bundle->first)) {
      std::cout << "Pre-side Bundle does not enter the post volume."
                << std::endl << std::endl;
      continue;
    }
