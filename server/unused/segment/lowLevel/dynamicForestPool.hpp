...
private:
    ...
    static uint64_t numBytes(const size_t s){
        return s * sizeof(Node);
    }
    ...
    void resize(const size_t newSize)
    {
        ...
        if(newSize <= size_){
            throw common::argException("we can only grow, shrinking doesn't make sense!!!");
        }
