...
template< class T >
struct floating_point_equals_predicate
{
    bool operator()( const T& lhs, const T& rhs ) const
    {
        const T diff      = std::abs( lhs - rhs );
        const T tolerance = std::numeric_limits<T>::epsilon() * 2;
        return ( diff <= tolerance );// * std::abs(lhs) && diff <= tolerance * std::abs(rhs) );
    }
};
...
struct floating_point_compare_predicate
{
    bool operator()( const T& lhs, const T& rhs ) const
    {
        floating_point_equals_predicate<T> equals;
...
template< class T, std::size_t N >
inline
typename detail::enable_if< is_floating_point< T >, bool >::type
same( const vec< T, N >& lhs, const vec< T, N >& rhs )
{
    return std::equal( lhs.begin(), lhs.end(), rhs.begin(), floating_point_equals_predicate<T>() );
}

template< class T, std::size_t N >
inline
typename detail::enable_if< is_integral< T >, bool >::type
same( const vec< T, N >& lhs, const vec< T, N >& rhs )
{
    return lhs == rhs;
}
...
struct hash< vec< T, N > >
{
    std::size_t operator()( const vec< T, N >& v ) const
    {
        std::size_t seed = 0;
        ::zi::hash<T> hasher;
        for ( std::size_t i = 0; i < N; ++i )
...
template< typename T, std::size_t N >
struct equal< vec< T, N > >
{
    bool operator()( const vec< T, N >& rhs, const vec< T, N >& lhs ) const
    {
        return same(lhs, rhs);
    }
};
