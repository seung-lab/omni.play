...
public:
    ...
    typedef vec< T, N >& type_reference;
...
#undef ZI_VL_COMPOUND_OPERATOR_TYPE
    ...
    // template< typename X >
    // typename detail::disable_if< ::zi::meta::and < is_same<T,X> , is_integral<X> >
    //                              , vec< X, N >& >::type
    ...
    // template< typename X >
    // typename detail::enable_if< ::zi::meta::and < is_same<T,X> , is_integral<X> >
    //                             , vec< X, N >& >::type
    type_reference operator/=( const T& rhs )
    {
        T rinv = static_cast< T >( 1 ) / rhs;
        for ( size_type i = 0; i < N; ++i )
        {
...
private:
    ...
    // void divide_by( const T& rhs,
    //                 typename detail::disable_if< is_integral< T >, T >::type* = 0 )
    // {
    //     T rinv = static_cast< T >( 1 ) / rhs;
    //     for ( size_type i = 0; i < N; ++i )
    //     {
    ...
    // typename detail::enable_if< is_integral< T >, void >::type
    // void divide_by( const T& rhs,
    //                 typename detail::enable_if< is_integral< T >, T >::type* = 0 )
    // {
    //     for ( size_type i = 0; i < N; ++i )
    //     {
    //         d[ i ] /= rhs;
