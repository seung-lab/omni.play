...
bool
OmMipVoxelationManager::ContainsVoxelation(const OmMipSegmentDataCoord &coord ) {
	return MipVoxelationCache::Contains(coord);
}

void 
OmMipVoxelationManager::RemoveVoxelation(const OmMipSegmentDataCoord &coord ) {
 ...
 */
bool
OmMipVoxelationManager::IsBoundaryVoxel( const DataCoord &rDataCoord ) {
	...
	for(int z=extent_min.z; z<=extent_max.z; ++z)
		for(int y=extent_min.y; y<=extent_max.y; ++y)
			for(int x=extent_min.x; x<=extent_max.x; ++x) {
				...
				if( mpMipVolume->GetVoxelValue(vox) != coord.DataValue) continue;
 ...
 */
void
OmMipVoxelationManager::RefreshNeighboringVoxels(const DataCoord &rVox, OmMipVoxelation &rVoxelation) {
	...
	for(int z=-1; z<=1; ++z)
		for(int y=-1; y<=1; ++y)
			for(int x=-1; x<=1; ++x) {
				...
				//if invalid, skip
				if(!mpMipVolume->ContainsVoxel(neighbor_coord)) continue;
				
				//remove voxel
				rVoxelation.RemoveVoxel(neighbor_coord);
				
				//if not part of segment, skip
				if( mpMipVolume->GetVoxelValue(neighbor_coord) != rVoxelation.GetSegmentDataValue() ) continue;
...
void
OmMipVoxelationManager::UpdateVoxel(const DataCoord &rVox, 
									const SEGMENT_DATA_TYPE old_val, 
									const SEGMENT_DATA_TYPE new_val ) {
...
void 
OmMipVoxelationManager::DrawVoxelations(OmSegmentManager &rSegMgr,
