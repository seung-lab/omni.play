    ...
    , sdw_(sdw)
{
    ...
    om::connect(colorButton, SIGNAL(clicked()),
            this, SLOT(setSegObjColor()));

    om::connect(nameEdit, SIGNAL(editingFinished()),
...
void SegObjectInspector::set_initial_values()
{
    ...
    const std::string listType = OmSegmentUtils::ListTypeAsStr(sdw_.GetSegment()->GetListType());
