...
void GLview::extract_data(int desired_depth)
{
	...
	DataBbox flat_data;
	if(ortho == "xy")
		flat_data = DataBbox(Vector3<int>(volume_extent.getMin().x, volume_extent.getMin().y, desired_depth), Vector3<int>(volume_extent.getMax().x, volume_extent.getMax().y, desired_depth));
	else if(ortho == "xz")
		flat_data = DataBbox(Vector3<int>(volume_extent.getMin().x, volume_extent.getMin().z, desired_depth), Vector3<int>(volume_extent.getMax().x, volume_extent.getMax().z, desired_depth));
	else
		flat_data = DataBbox(Vector3<int>(volume_extent.getMin().z, volume_extent.getMin().y, desired_depth), Vector3<int>(volume_extent.getMax().z, volume_extent.getMax().y, desired_depth));
			...
			for(int x = start_mipcoord.get<1>() ; x <= end_mipcoord.get<1>() ; x++)
			{
				for(int i = 0 ; i <= 0 ; i++) {
					...
					if(!(mastermap.contains(OmMipChunkCoord(level, x, y, desired_depth + i)))) {
						...
						mastermap.insert((OmMipChunkCoord(level, x, y, desired_depth + i)), tex_num);
						...
						if(DEBUG) {
							...
							cout << "DEPTH DEPTH DEPTH ORTHO = " << desired_depth + i << " " << ortho << endl;
	...
	while (i.hasNext()) {
		...
		if(i.key().get<0>() == level) {
			...
			if(ortho != "yz") {
				...
				glVertex3f(i.key().get<1>() * tileLength, i.key().get<2>() * tileLength, (i.key().get<3>() * 2.0));
				glTexCoord2f (1.0f, 0.0f); /* lower right corner of image */
				glVertex3f((i.key().get<1>() * tileLength + tileLength), i.key().get<2>() * tileLength, (i.key().get<3>() * 2.0));
				glTexCoord2f (1.0f, 1.0f); /* upper right corner of image */
				glVertex3f((i.key().get<1>() * tileLength + tileLength), (i.key().get<2>() * tileLength + tileLength),  (i.key().get<3>() * 2.0));
				glTexCoord2f (0.0f, 1.0f); /* upper left corner of image */
				glVertex3f(i.key().get<1>() * tileLength, (i.key().get<2>() * tileLength + tileLength), (i.key().get<3>() * 2.0));
				glEnd ();
			}
			else if(ortho == "yz") {
				glTexCoord2f (0.0f,0.0f); /* lower left corner of image */
		...
		for (int y = y_min ; y <= y_max ; y++) {
			for (int x = x_min; x <= x_max ; x++) {
				...
				if(id == 0) {
					...
					const Vector3<float> &color = OmVolume::GetSegmentation(segmentation_id).GetSegment(id).GetColor();
					...
					if(mSamplesPerVoxel == 1) {
						...
						data[ctr] = (newcolor.red() * 0.5) + (((unsigned int) imageData[ctr]) * (1.0 - 0.5));
						data[ctr+1] = (newcolor.green() * 0.5) + (((unsigned int) imageData[ctr+1]) * (1.0 - 0.5));
						data[ctr+2] = (newcolor.blue() * 0.5) + (((unsigned int) imageData[ctr+2]) * (1.0 - 0.5));
						data[ctr+3] = 255;
