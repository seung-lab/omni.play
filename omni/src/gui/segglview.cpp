...
SegGLview::SegGLview(QWidget *parent) : QGLWidget(QGLFormat(QGL::DoubleBuffer|QGL::DepthBuffer|QGL::AlphaChannel), parent)
{
...
void SegGLview::extract_data(int desired_depth)
{
	...
	DataBbox flat_data;
	if(ortho == "xy")
		flat_data = DataBbox(Vector3<int>(channel_extent.getMin().x, channel_extent.getMin().y, desired_depth), Vector3<int>(channel_extent.getMax().x, channel_extent.getMax().y, desired_depth));
	else if(ortho == "xz")
		flat_data = DataBbox(Vector3<int>(channel_extent.getMin().x, channel_extent.getMin().z, desired_depth), Vector3<int>(channel_extent.getMax().x, channel_extent.getMax().z, desired_depth));
	else
		flat_data = DataBbox(Vector3<int>(channel_extent.getMin().z, channel_extent.getMin().y, desired_depth), Vector3<int>(channel_extent.getMax().z, channel_extent.getMax().y, desired_depth));
	...
	if(ortho == "xy")
		adjusted_data_to_retrieve = DataBbox(Vector3<int>(data_to_retrieve.getMin().x, data_to_retrieve.getMin().y, data_to_retrieve.getMin().z), Vector3<int>(data_to_retrieve.getMax().x, data_to_retrieve.getMax().y, data_to_retrieve.getMax().z));
	else if(ortho == "xz")
		adjusted_data_to_retrieve = DataBbox(Vector3<int>(data_to_retrieve.getMin().x, data_to_retrieve.getMin().z, data_to_retrieve.getMin().y), Vector3<int>(data_to_retrieve.getMax().x, data_to_retrieve.getMax().z, data_to_retrieve.getMax().y));
	else
		adjusted_data_to_retrieve = DataBbox(Vector3<int>(data_to_retrieve.getMin().z, data_to_retrieve.getMin().y, data_to_retrieve.getMin().x), Vector3<int>(data_to_retrieve.getMax().z, data_to_retrieve.getMax().y, data_to_retrieve.getMax().x));
			...
			for(int x = start_mipcoord.get<1>() ; x <= end_mipcoord.get<1>() ; x++)
			{
				for(int i = 0 ; i <= 0 ; i++) {
					...
					if(!(channelmap.contains(OmMipChunkCoord(level, x, y, desired_depth + i)))) {
						...
						void* void_data;
						void_data = my_chunk->ExtractDataSlice(VOL_XY_PLANE, actual_depth, tile_dims);
						
						
						char* data = (char*)void_data;
						
						int tex_num = setTexture(data, tile_dims, OmMipChunkCoord(mylevel, x, y, end_mipcoord.get<3>()), desired_depth + i);
						if(DEBUG)
							cout << "tile dims: " << tile_dims << endl;
						delete data;
						
						
						channelmap.insert((OmMipChunkCoord(level, x, y, desired_depth + i)), tex_num);
						
						if(DEBUG)
							cout << "inserted " << (OmMipChunkCoord(level, x, y, desired_depth + i)) << endl;
					...
					if(!(segmap.contains(OmMipChunkCoord(level, x, y, desired_depth + i)))) {
						...
						void* void_c_data;
						void_data = my_chunk->ExtractDataSlice(VOL_XY_PLANE, actual_depth, tile_dims);
						void_c_data = my_c_chunk->ExtractDataSlice(VOL_XY_PLANE, actual_depth, tile_dims);
						
						
						char* data = (char*)void_data;
						char* c_data = (char*)void_c_data;
						
						int tex_num = setSegTexture(data, c_data, tile_dims, OmMipChunkCoord(mylevel, x, y, end_mipcoord.get<3>()), desired_depth + i);
						if(DEBUG)
							cout << "tile dims: " << tile_dims << endl;
						delete data;
						delete c_data;
						
						segmap.insert((OmMipChunkCoord(level, x, y, desired_depth + i)), tex_num);
						...
						if(DEBUG) {
							...
							cout << "mastermap does contain " << (OmMipChunkCoord(level, x, y, desired_depth + i)) << endl;
						}
				}
			...
			for(int x = start_mipcoord.get<1>() ; x <= end_mipcoord.get<1>() ; x++)
			{
				for(int i = 0 ; i <= 0 ; i++) {
					if(!(channelmap.contains(OmMipChunkCoord(level, x, y, desired_depth + i)))) {
						...
						Vector2<int> tile_dims;
						void* void_data;
						void_data = my_chunk->ExtractDataSlice(VOL_XZ_PLANE, actual_depth, tile_dims);
						
						
						char* data = (char*)void_data;
						
						int tex_num = setTexture(data, tile_dims, OmMipChunkCoord(mylevel, x, end_mipcoord.get<2>(), y), desired_depth + i);
						if(DEBUG)
							cout << "tile dims: " << tile_dims << endl;
						delete data;
					...
					if(!(segmap.contains(OmMipChunkCoord(level, x, y, desired_depth + i)))) {
						...
						void* void_c_data;
						void_data = my_chunk->ExtractDataSlice(VOL_XZ_PLANE, actual_depth, tile_dims);
						void_c_data = my_chunk->ExtractDataSlice(VOL_XZ_PLANE, actual_depth, tile_dims);
						
						char* data = (char*)void_data;
						char* c_data = (char*)void_c_data;

						
						int tex_num = setSegTexture(data, c_data, tile_dims, OmMipChunkCoord(mylevel, x, end_mipcoord.get<2>(), y), desired_depth + i);
						if(DEBUG)
							cout << "tile dims: " << tile_dims << endl;
						delete data;
						delete c_data;
			...
			for(int x = start_mipcoord.get<3>() ; x <= end_mipcoord.get<3>() ; x++)
			{
				...
				for(int i = 0 ; i <= 0 ; i++) {
					if(!(channelmap.contains(OmMipChunkCoord(level, x, y, desired_depth + i)))) {
						...
						Vector2<int> tile_dims;
						void* void_data;
						void_data = my_chunk->ExtractDataSlice(VOL_YZ_PLANE, actual_depth, tile_dims);
						
						
						char* data = (char*)void_data;
						
						int tex_num = setTexture(data, tile_dims, (OmMipChunkCoord(mylevel, end_mipcoord.get<1>(), y, x)), desired_depth + i);
						if(DEBUG)
							cout << "tile dims: " << tile_dims << endl;
						delete data;
					...
					if(!(segmap.contains(OmMipChunkCoord(level, x, y, desired_depth + i)))) {
						...
						void* void_c_data;
						void_data = my_chunk->ExtractDataSlice(VOL_YZ_PLANE, actual_depth, tile_dims);
						void_c_data = my_c_chunk->ExtractDataSlice(VOL_YZ_PLANE, actual_depth, tile_dims);

						
						char* data = (char*)void_data;
						char* c_data = (char*)void_c_data;
						
						int tex_num = setSegTexture(data, c_data, tile_dims, (OmMipChunkCoord(mylevel, end_mipcoord.get<1>(), y, x)), desired_depth + i);
						if(DEBUG)
							cout << "tile dims: " << tile_dims << endl;
						delete data;
						delete c_data;
...
int SegGLview::setSegTexture(char *imageData, char *channelData, Vector2<int>  dims,  const OmMipChunkCoord &mipCoord, int depth)
{
	// glEnable (GL_DEPTH_TEST);
    glPolygonMode (GL_FRONT_AND_BACK, GL_FILL);
	...
	while (is.hasNext()) {
		...
		if(is.key().get<0>() == level) {
			...
			if(ortho != "yz") {
				...
				glTexCoord2f (0.0f,0.0f); /* lower left corner of image */
				glVertex3f(is.key().get<1>() * seg_tileLength, is.key().get<2>() * seg_tileLength, (is.key().get<3>() * 2.0));
				glTexCoord2f (1.0f, 0.0f); /* lower right corner of image */
				glVertex3f((is.key().get<1>() * seg_tileLength + seg_tileLength), is.key().get<2>() * seg_tileLength, (is.key().get<3>() * 2.0));
				glTexCoord2f (1.0f, 1.0f); /* upper right corner of image */
				glVertex3f((is.key().get<1>() * seg_tileLength + seg_tileLength), (is.key().get<2>() * seg_tileLength + seg_tileLength),  (is.key().get<3>() * 2.0));
				glTexCoord2f (0.0f, 1.0f); /* upper left corner of image */
				glVertex3f(is.key().get<1>() * seg_tileLength, (is.key().get<2>() * seg_tileLength + seg_tileLength), (is.key().get<3>() * 2.0));
				glEnd ();
			}
			else if(ortho == "yz") {
				glTexCoord2f (0.0f,0.0f); /* lower left corner of image */
...
#pragma mark <TODO: make this more sane>
	...
	if((level + future_level) > level) {
		...
		if(DEBUG)
			cout << "view->camera: (" << view->shadow_camera.x << ", " << view->shadow_camera.y << ")   "  << "w: " << view->shadow_camera.w << "   h: " << view->shadow_camera.h << endl;
...
void SegGLview::setMyColorMap(char *imageData, char *channelData, Vector2<int>  dims,  const OmMipChunkCoord &mipCoord, int depth)
{
...
//				cout << "volume_id = " << volume_id << endl;
				OmSegmentation &current_seg = OmVolumeManager::GetVolume(volume_id).GetSegmentation(seg_id);
...
//				cout << "vector = " << Vector3<int>(x,y,z) << endl;
				...
				if(id == 0) {
					...
					const Vector3<float> &color = OmVolumeManager::GetVolume(volume_id).GetSegmentation(seg_id).GetSegment(id).GetColor();
