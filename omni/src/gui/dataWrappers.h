...
class SegmentDataWrapper:public DataWrapper {
 public:
	SegmentDataWrapper() {
	} SegmentDataWrapper(const OmId segmentationID, const OmId segmentID);
...
class FilterDataWrapper:public DataWrapper {
 public:
	FilterDataWrapper() {
	} FilterDataWrapper(const OmId channelID, const OmId mID, const QString name, const bool enabled);
	OmId getChannelID() {
		return mChannelID;
	}
	QString getNote();
