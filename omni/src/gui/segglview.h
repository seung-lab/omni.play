...
class SegGLview : public QGLWidget
{
	...
	private:
		...
		int setSegTexture(char *imageData, char *channelData,  Vector2<int> dims, const OmMipChunkCoord &mipCoord, int depth);
		void draw();
		...
		void setMyColorMap(char *imageData, char *channelData, Vector2<int> dims, const OmMipChunkCoord &mipCoord, int depth);
