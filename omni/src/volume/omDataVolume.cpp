...
const Vector3<int> 
OmDataVolume::GetDimensions() {
	return GetExtent().getUnitDimensions();
}

void 
OmDataVolume::SetBytesPerSample(int bytesPerSample) {
	mBytesPerSample = bytesPerSample;
}

const int 
OmDataVolume::GetBytesPerSample() {
	return mBytesPerSample;
}


void 
OmDataVolume::SetOpen(bool state) {
	mIsOpen = state;
}
...
bool 
OmDataVolume::IsOpen() const { 
	return mIsOpen;
}

bool
OmDataVolume::IsDirty() const { 
	assert(false);
	return false;
}
...
bool 
OmDataVolume::ContainsVoxel(const DataCoord &vox) {
...
ostream& 
operator<<(ostream &out, const OmDataVolume &v) {
