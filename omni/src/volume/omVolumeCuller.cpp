 ...
 */
bool
OmVolumeCuller::CheckDrawOption(OmBitfield option) const {
	return mOptionBits & option;
}

OmBitfield
OmVolumeCuller::MaskDrawOptions(OmBitfield mask) const {
	return mOptionBits & mask;
}

OmBitfield 
OmVolumeCuller::GetDrawOptions() const {
	return mOptionBits;
}


const NormCoord& 
OmVolumeCuller::GetPosition() const { 
	return mPosition;
}

const NormCoord& 
OmVolumeCuller::GetFocus() const { 
	return mFocus;
}
...
OmVolumeCuller 
OmVolumeCuller::GetTransformedCuller(const Matrix4f& mat, const Matrix4f& matInv) const {
...
void 
OmVolumeCuller::TransformCuller(const Matrix4f& mat, const Matrix4f& matInv) {
...
Visibility
OmVolumeCuller::TestChunk(const OmMipChunk &rChunk) const {
	return mFrustumCuller.testAabb(rChunk.GetNormExtent());
}

Visibility
OmVolumeCuller::VisibilityTestNormBbox(const NormBbox &bbox) const {
