...
//mip level method
	...
	Vector3i MipLevelDimensionsInMipChunks(int level) const
	{
	 ...
	 */
	OmChunkCoord DataToMipCoord(const DataCoord& dataCoord,
								const int level) const
	{
		return DataToMipCoord(dataCoord, level, GetChunkDimensions());
	}

	static OmChunkCoord DataToMipCoord(const DataCoord& dataCoord,
									   const int level,
	...
	OmChunkCoord NormToMipCoord(const NormCoord &normCoord,
								const int level) const
	{
	 ...
	 */
	DataBbox MipCoordToDataBbox(const OmChunkCoord & rMipCoord,
	...
	//mip chunk methods
	OmChunkCoord RootMipChunkCoordinate() const {
	 ...
	 */
	bool ContainsMipChunkCoord(const OmChunkCoord & rMipCoord) const
	{
	 ...
	 */
	void ValidMipChunkCoordChildren(const OmChunkCoord & rMipCoord,
	...
	uint32_t ComputeTotalNumChunks() const
	{
	...
	uint32_t ComputeTotalNumChunks(const int mipLevel) const
	{
		const Vector3i dims = MipLevelDimensionsInMipChunks(mipLevel);
		return dims.x * dims.y * dims.z;
	}

	uint32_t GetNumberOfVoxelsPerChunk() const
	{
