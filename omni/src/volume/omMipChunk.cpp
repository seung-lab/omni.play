...
bool 
OmMipChunk::IsDirty() const {
	return IsVolumeDataDirty() || IsMetaDataDirty();
}

bool 
OmMipChunk::IsVolumeDataDirty() const {
	return mVolumeDataDirty;
}

bool 
OmMipChunk::IsMetaDataDirty() const {
...
OmMipChunk::WriteVolumeData() {
	...
	if (mpImageData) OmProjectData::WriteImageData(mip_level_vol_path, GetExtent(), GetBytesPerSample(), mpImageData);
	//data clean
	mVolumeDataDirty = false;
}
...
OmMipChunk::GetModifiedVoxelValues() const {
	return mModifiedVoxelValues;
}

void
OmMipChunk::ClearModifiedVoxelValues() {
 ...
 */
void 
OmMipChunk::SetVoxelValue(const DataCoord &voxel, uint32_t val) {
 ...
 */
uint32_t
OmMipChunk::GetVoxelValue(const DataCoord &voxel) {
	//assert valid
	assert(ContainsVoxel(voxel));
 ...
 */
void 
OmMipChunk::SetImageData(vtkImageData *pImageData) {
...
int 
OmMipChunk::GetLevel() const {
	return mCoordinate.Level;
}

bool 
OmMipChunk::IsRoot() const {
	//if parent is null
	return mParentCoord == OmMipChunkCoord::NULL_COORD;
}

bool 
OmMipChunk::IsLeaf() const {
	return mCoordinate.Level == 0;
}


const OmMipChunkCoord& 
OmMipChunk::GetCoordinate() { 
	return mCoordinate; 
}

const OmMipChunkCoord& 
OmMipChunk::GetParentCoordinate() { 
	return mParentCoord;
}

const set<OmMipChunkCoord>&
OmMipChunk::GetChildrenCoordinates() { 
...
const NormBbox& 
OmMipChunk::GetNormExtent() const {
	return mNormExtent;
}


const NormBbox& 
OmMipChunk::GetClippedNormExtent() const {
 ...
 */
const SegmentDataSet& 
OmMipChunk::GetDirectDataValues() const { 
 ...
 */
const SegmentDataSet& 
OmMipChunk::GetIndirectDataValues() const { 
 ...
 */
void
OmMipChunk::RefreshDirectDataValues() {
 ...
 */
void
OmMipChunk::RefreshIndirectDataValues() {
	...
	for(itr = mChildrenCoordinates.begin(); itr != mChildrenCoordinates.end(); itr++) {
		...
		mIndirectlyContainedDataValuesSet.insert(r_child_indirect_data_values.begin(), r_child_indirect_data_values.end());
...
OmMipChunk::ExtractSliceExtent(OmDataVolumePlane plane, int coord) {
	//ensure open
	Open();
	...
	for(int z=0; z<2; z++)
		for(int y=0; y<2; y++)
			for(int x=0; x<2; x++) {
				...
				copyIntersectedImageDataFromOffset(p_mesh_data,
												   p_src_data, offset);
			}
...
bool 
OmMipChunk::DrawCheck( const OmVolumeCuller &rCuller ) {
	//draw if leaf
...
ostream& 
operator<<(ostream &out, const OmMipChunk &mc) {
