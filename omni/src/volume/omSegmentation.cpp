...
void 
OmSegmentation::SetDirectoryPath(string dpath) {
	//call parent
 ...
 */
OmId 
OmSegmentation::GetSegmentIdMappedToValue( SEGMENT_DATA_TYPE value ) {
...
OmSegmentation::GetValuesMappedToSegmentId( OmId omId ) {
 ...
 */
SEGMENT_DATA_TYPE 
OmSegmentation::GetValueMappedToSegmentId( OmId omId ) {
...
void
OmSegmentation::MapValuesToSegmentId( OmId omId, const SegmentDataSet &values) {
	mSegmentManager.MapValuesToSegmentId(omId, values);
}


void
OmSegmentation::UnMapValuesToSegmentId( OmId omId, const SegmentDataSet &values) {
 ...
 */
void 
OmSegmentation::SetVoxelValue(const DataCoord &rVox, uint32_t val) {
 ...
 */
OmId 
OmSegmentation::GetVoxelSegmentId(const DataCoord &vox) {
 ...
 */
void 
OmSegmentation::SetVoxelSegmentId(const DataCoord &vox, OmId omId) {
...
bool
OmSegmentation::IsVolumeDataBuilt() {
	return OmMipVolume::IsBuilt();
}

void
OmSegmentation::BuildVolumeData() {
 ...
 */
bool
OmSegmentation::IsMeshDataBuilt() {
 ...
 */
void 
OmSegmentation::BuildMeshData() {
	
	if(!IsVolumeDataBuilt())
		throw OmAccessException(string("Segmentation volume data must be built before mesh data: ") + GetName() );
...
void 
OmSegmentation::BuildMeshDataInternal() {
 ...
 */
void 
OmSegmentation::BuildChunk(const OmMipChunkCoord &mipCoord) {
...
void 
OmSegmentation::RebuildChunk(const OmMipChunkCoord &mipCoord, const SegmentDataSet &rModifiedValues) {
 ...
 */
void 
OmSegmentation::ExportDataFilter(vtkImageData *pImageData) {
...
void
OmSegmentation::SystemModeChangeEvent(OmSystemModeEvent *event) {
...
OmSegment& 
OmSegmentation::GetSegment(OmId id) {
	return mSegmentManager.GetSegment(id);
}

OmSegment& 
OmSegmentation::AddSegment() {
	return mSegmentManager.AddSegment();
}

void 
OmSegmentation::RemoveSegment(OmId id) {
	mSegmentManager.RemoveSegment(id);
}

bool 
OmSegmentation::IsSegmentValid(OmId omId) {
	return mSegmentManager.IsSegmentValid(omId);
}

const set<OmId>& 
OmSegmentation::GetValidSegmentIds() {
	return mSegmentManager.GetValidSegmentIds();
}

bool 
OmSegmentation::IsSegmentEnabled(OmId id) {
	return mSegmentManager.IsSegmentEnabled(id);
}

void 
OmSegmentation::SetSegmentEnabled(OmId id, bool enable) {
	mSegmentManager.SetSegmentEnabled(id, enable);
}

const set<OmId>&
OmSegmentation::GetEnabledSegmentIds() {
	return mSegmentManager.GetEnabledSegmentIds();
}

bool 
OmSegmentation::IsSegmentSelected(OmId id) {
	return mSegmentManager.IsSegmentSelected(id);
}

void 
OmSegmentation::SetSegmentSelected(OmId id, bool selected) {
...
const OmIds&
OmSegmentation::GetSelectedSegmentIds() {
...
const SegmentDataSet&
OmSegmentation::GetEnabledSegmentDataValues() {
	return mSegmentManager.GetEnabledSegmentDataValues();
}

const SegmentDataSet&
OmSegmentation::GetSelectedSegmentDataValues() {
	return mSegmentManager.GetSelectedSegmentDataValues();
}


const SegmentDataSet&
OmSegmentation::GetUnselectedSegmentDataValues() {
 ...
 */
void 
OmSegmentation::Draw(const OmVolumeCuller &rCuller) {
	
	//return if mesh data not yet built
	if(!IsMeshDataBuilt()) return;
	...
	if(0 == p_relv_data_vals->size()) return;
...
OmSegmentation::DrawChunkRecursive(const OmMipChunkCoord &chunkCoord, const SegmentDataSet &rRelvDataVals, bool testVis,
	...
	if(p_chunk->DrawCheck(rCuller)) {
		...
		setIntersection< SEGMENT_DATA_TYPE >(rRelvDataVals, p_chunk->GetDirectDataValues(), direct_relevant_data_set);
		
		//return if empty
		if(direct_relevant_data_set.size() == 0) return;
	...
	setIntersection< SEGMENT_DATA_TYPE >(rRelvDataVals, p_chunk->GetIndirectDataValues(), indirect_relevant_data_set);
	
	//return if empty
	if(indirect_relevant_data_set.size() == 0) return;
...
OmSegmentation::DrawChunk(const OmMipChunkCoord &chunkCoord, const SegmentDataSet &rRelvDataVals, const OmVolumeCuller &rCuller) {
	...
	if( (OmStateManager::GetSystemMode() == EDIT_SYSTEM_MODE) && p_chunk->IsLeaf() ) {
		...
		setIntersection< SEGMENT_DATA_TYPE >(rRelvDataVals, GetSelectedSegmentDataValues(), selected_relevant_data_set);
		...
		setIntersection< SEGMENT_DATA_TYPE >(rRelvDataVals, GetUnselectedSegmentDataValues(), unselected_relevant_data_set);
 ...
 */
void 
OmSegmentation::DrawChunkMeshes( const OmMipChunkCoord &mipCoord, const SegmentDataSet &rRelvDataVals, const OmBitfield &drawOps ) {
 ...
 */
void
OmSegmentation::DrawChunkVoxels( const OmMipChunkCoord &mipCoord, const SegmentDataSet &rRelvDataVals, const OmBitfield &drawOps ) {
