...
void 
OmView3d::mousePressEvent(QMouseEvent *event) {
	try {
...
void 
OmView3d::mouseMoveEvent(QMouseEvent *event) { 
	mView3dUi.MouseMove(event);
} 

void 
OmView3d::mouseDoubleClickEvent(QMouseEvent *event) { 
	mView3dUi.MouseDoubleClick(event);
} 

void 
OmView3d::keyPressEvent(QKeyEvent *event) {
 ...
 */
void
OmView3d::PreferenceChangeEvent(OmPreferenceEvent *event) {
...
void 
OmView3d::SegmentObjectModificationEvent(OmSegmentEvent *event) {
	updateGL();
}

void
OmView3d::VoxelModificationEvent(OmVoxelEvent *event) {
	updateGL();
}

void 
OmView3d::SegmentDataModificationEvent(OmSegmentEvent *event) {
	updateGL();	
}

void 
OmView3d::SystemModeChangeEvent(OmSystemModeEvent *event) {
	updateGL();
}


void
OmView3d::ViewBoxChangeEvent(OmViewEvent *event) { 
	updateGL();
} 

void 
OmView3d::View3dRedrawEvent(OmView3dEvent *event) {
	updateGL();
}

void
OmView3d::View3dUpdatePreferencesEvent(OmView3dEvent *event) {
	//UpdateEnabledWidgets();
	//updateGL();
}
 ...
 */
bool 
OmView3d::PickPoint(Vector2<int> point2d, vector<int> &rNamesVec) {
	//clear name vector
	rNamesVec.clear();
...
bool
OmView3d::UnprojectPoint(Vector2i point2d, Vector3f &point3d, float z_scale_factor) {
 ...
 */
void 
OmView3d::Draw(OmBitfield cullerOptions) {
	...
	if(cullerOptions & DRAWOP_LEVEL_VOLUME) {
		...
		if(cullerOptions & DRAWOP_RENDERMODE_RENDER) {
			...
			if(OmPreferences::GetBoolean(OM_PREF_VIEW3D_TRANSPARENT_UNSELECTED_BOOL)) glDepthMask(GL_FALSE);
			//draw unselected segments
			DrawVolumes(cullerOptions | DRAWOP_SEGMENT_FILTER_UNSELECTED | DRAWOP_SEGMENT_COLOR_TRANSPARENT);
 ...
 */
void 
OmView3d::DrawVolumes(OmBitfield cullerOptions) {
...
void 
OmView3d::DrawEditSelectionVoxels() {
