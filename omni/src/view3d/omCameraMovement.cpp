...
void OmCameraArcBall::Drag(const Vector2 < float >&point)
{
	...
	if (perp_vector.length() > std::numeric_limits< float >::epsilon() ) {	//if non-zero
		...
		rotation =
		    Quaternion < float >(Vector3 < float >::dot(mStartVector, mEndVector), perp_vector)*mStartRotation;
		rotation.normalize();
 ...
 */
void OmCameraArcBall::MapPointToSphere(const Vector2 < float >&point, Vector3 < float >&vector)
{
	//get viewport and scale
	const Vector4 < int >&r_viewport = mpCamera->mViewport;
	//assert((viewport.width > 1) && (viewport.height > 1));
	if ((r_viewport.width < 1) || (r_viewport.height < 1))
		return;
