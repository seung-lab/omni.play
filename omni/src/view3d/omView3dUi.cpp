...
void 
OmView3dUi::MouseRelease(QMouseEvent *event) {
	switch(OmStateManager::GetSystemMode()) {
		case NAVIGATION_SYSTEM_MODE:
...
void
OmView3dUi::MouseMove(QMouseEvent *event) {
...
void 
OmView3dUi::MouseDoubleClick(QMouseEvent *event) {
	switch(OmStateManager::GetSystemMode()) {
		case NAVIGATION_SYSTEM_MODE:
...
void 
OmView3dUi::KeyPress(QKeyEvent *event) {
	switch(OmStateManager::GetSystemMode()) {
		case NAVIGATION_SYSTEM_MODE:
...
void 
OmView3dUi::NavigationModeMousePressed(QMouseEvent *event) {
...
void 
OmView3dUi::NavigationModeMouseRelease(QMouseEvent *event) {
...
void 
OmView3dUi::NavigationModeMouseDoubleClick(QMouseEvent *event) {
	SegmentSelectToggleMouse(event, false);
}

void 
OmView3dUi::NavigationModeKeyPress(QKeyEvent *event) {
...
void 
OmView3dUi::EditModeMousePressed(QMouseEvent *event) {
...
void 
OmView3dUi::EditModeMouseMove(QMouseEvent *event) {
...
void 
OmView3dUi::EditModeMouseDoubleClick(QMouseEvent *event) {
	SegmentSelectToggleMouse(event, false);
}


void 
OmView3dUi::EditModeKeyPress(QKeyEvent *event) {
...
void 
OmView3dUi::CameraMovementMouseStart(QMouseEvent *event) {
...
void 
OmView3dUi::CameraMovementMouseEnd(QMouseEvent *event) {
	Vector2f point = Vector2f( event->x(), event->y() );
...
void 
OmView3dUi::CameraMovementMouseUpdate(QMouseEvent *event) {
	Vector2f point = Vector2f( event->x(), event->y() );
...
bool 
OmView3dUi::PickSegmentMouse(QMouseEvent *event, bool drag, OmId &segmentationId, OmId &segmentId, int *type) {
...
bool
OmView3dUi::PickVoxel(QKeyEvent *keyEvent, QMouseEvent *mouseEvent, bool drag, DataCoord &voxel) {
	if(NULL != keyEvent) {
...
bool
OmView3dUi::PickVoxelMouse(QMouseEvent *event, bool drag, DataCoord &rVoxel) {
...
bool
OmView3dUi::PickVoxelCameraFocus(QKeyEvent *keyEvent, bool drag, DataCoord &rVoxel) {
...
void 
OmView3dUi::SegmentSelectToggleMouse(QMouseEvent *event, bool drag) {
	...
	if(!PickSegmentMouse(event, drag, segmentation_id, segment_id, &pick_object_type)) return;
	
	//if picked type was not a mesh
	if(pick_object_type != OMGL_NAME_MESH) return;
	...
	if(!augment_selection && new_segment_select_state) {
		...
		(new OmSegmentSelectAction(segmentation_id, select_segment_ids, r_segmentation.GetSelectedSegmentIds()))->Run();
...
void 
OmView3dUi::VoxelEditMouse(QMouseEvent *mouseEvent, bool drag) {
...
void 
OmView3dUi::VoxelSelectToggleMouse(QMouseEvent *mouseEvent, bool drag) {
	...
	if(!PickVoxelMouse(mouseEvent, drag, picked_voxel)) return;
...
void 
OmView3dUi::VoxelSelectToggleKey(QKeyEvent *keyEvent, bool augment) {
		
	//pick voxel
	DataCoord picked_voxel;
	if(!PickVoxelCameraFocus(keyEvent, false, picked_voxel)) return;
...
void 
OmView3dUi::VoxelSetMouse(QMouseEvent *mouseEvent, bool drag) {
...
void
OmView3dUi::ShowSegmentContextMenu(QMouseEvent *event) {
	
	//get segment
	OmId segmentation_id, segment_id;
	if(!PickSegmentMouse(event, false, segmentation_id, segment_id)) return;
