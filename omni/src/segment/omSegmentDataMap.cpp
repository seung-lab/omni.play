...
bool 
OmSegmentDataMap::IsValueMappedToSegmentId( SEGMENT_DATA_TYPE value ) {
	return mDataValueToId.count(value);
}


SEGMENT_DATA_TYPE 
OmSegmentDataMap::GetUnMappedValue() {
...
OmId
OmSegmentDataMap::GetSegmentIdMappedToValue( SEGMENT_DATA_TYPE value ) {
	return mDataValueToId[value];
}


const SegmentDataSet&
OmSegmentDataMap::GetValuesMappedToSegmentId( OmId omId ) {
 ...
 */
void 
OmSegmentDataMap::MapValueToSegmentId( OmId omId, SEGMENT_DATA_TYPE value) {
...
void
OmSegmentDataMap::MapValuesToSegmentId( OmId omId, const SegmentDataSet &values ) {
...
void
OmSegmentDataMap::UnMapValuesToSegmentId( OmId omId, const SegmentDataSet &values) {
 ...
 */
void 
OmSegmentDataMap::UnMapSegmentId( OmId omId ) {
...
void 
OmSegmentDataMap::UnMapValues( const SegmentDataSet &values) {
	
	//for each data value
	SegmentDataSet::iterator itr;
	for(itr = values.begin(); itr != values.end(); itr++) {
		
		if(mDataValueToId.count(*itr) == 0) assert(false);
