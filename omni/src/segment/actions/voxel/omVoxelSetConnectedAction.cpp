...
void
OmVoxelSetConnectedAction::AddConnectedNeighborsToList(OmSegmentation &rSegmentation, const DataCoord &srcVox, list< DataCoord >& todoList) {
	...
	for(int z=-1; z<=1; ++z)
		for(int y=-1; y<=1; ++y)
			for(int x=-1; x<=1; ++x) {
				...
				if(!rSegmentation.ContainsVoxel(offset_vox)) continue;
...
void 
OmVoxelSetConnectedAction::Action() {
...
void 
OmVoxelSetConnectedAction::UndoAction()	{ 
	assert(false);
}


string 
OmVoxelSetConnectedAction::Description() {
