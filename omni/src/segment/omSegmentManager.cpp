...
OmSegment& 
OmSegmentManager::GetSegment(OmId id) {
	return mGenericSegmentManager.Get(id);
}

OmSegment& 
OmSegmentManager::AddSegment() {
	mCachedSegmentDataValuesDirty = true;
	...
	if(NULL_SEGMENT_DATA == data_value) assert(false);
...
OmSegment& 
OmSegmentManager::AddSegment(SEGMENT_DATA_TYPE data_value) {
	mCachedSegmentDataValuesDirty = true;
	
	//check value already mapped or there are no more free values
	if(IsValueMappedToSegmentId(data_value)) assert(false);
...
void 
OmSegmentManager::AddSegments(const SegmentDataSet& data_values) {
	...
	for( itr = data_values.begin(); itr != data_values.end(); itr++ ) {
		...
		if(IsValueMappedToSegmentId( segment_data_value )) continue;
...
void 
OmSegmentManager::RemoveSegment(OmId id) {
	mGenericSegmentManager.Remove(id);
}


bool 
OmSegmentManager::IsSegmentValid(OmId omId) const {
	return mGenericSegmentManager.IsValid(omId);
}

const set<OmId>& 
OmSegmentManager::GetValidSegmentIds() {
	return mGenericSegmentManager.GetValidIds();
}

bool 
OmSegmentManager::IsSegmentEnabled(OmId id) {
	return mGenericSegmentManager.IsEnabled(id);
}

void 
OmSegmentManager::SetSegmentEnabled(OmId id, bool enable) {
	mCachedSegmentDataValuesDirty = true;
	mGenericSegmentManager.SetEnabled(id, enable);
}

const set<OmId>&
OmSegmentManager::GetEnabledSegmentIds() {
	return mGenericSegmentManager.GetEnabledIds();
}

bool 
OmSegmentManager::IsSegmentSelected(OmId id) {
	return mGenericSegmentManager.IsSelected(id);
}

void 
OmSegmentManager::SetSegmentSelected(OmId id, bool selected) {
	mCachedSegmentDataValuesDirty = true;
	mGenericSegmentManager.SetSelected(id, selected);
}

void 
OmSegmentManager::SetAllSegmentsSelected(bool selected) {
	mCachedSegmentDataValuesDirty = true;
...
uint32_t 
OmSegmentManager::SegmentSize() {
	return mGenericSegmentManager.Size();
}

void 
OmSegmentManager::SegmentCall(void (OmSegment::*fxn)() ) {
...
bool 
OmSegmentManager::IsValueMappedToSegmentId( SEGMENT_DATA_TYPE value ) {
	return mSegmentDataMap.IsValueMappedToSegmentId(value);
}

OmId 
OmSegmentManager::GetSegmentIdMappedToValue( SEGMENT_DATA_TYPE value ) {
	mSegmentDataMap.GetSegmentIdMappedToValue(value);
}

const SegmentDataSet& 
OmSegmentManager::GetValuesMappedToSegmentId( OmId omId ) {
	mSegmentDataMap.GetValuesMappedToSegmentId(omId);
}


void 
OmSegmentManager::MapValueToSegmentId( OmId omId, SEGMENT_DATA_TYPE value ) {
	mSegmentDataMap.MapValueToSegmentId(omId, value);
}

void 
OmSegmentManager::MapValuesToSegmentId( OmId omId, const SegmentDataSet &rValueSet ) {
	mSegmentDataMap.MapValuesToSegmentId(omId, rValueSet);
}

void 
OmSegmentManager::UnMapValuesToSegmentId( OmId omId, const SegmentDataSet &rValueSet ) {
...
void
OmSegmentManager::ConvertSegmentIdsToDataValues( const OmIds& omIds, SegmentDataSet& values) {
...
void
OmSegmentManager::RefreshCachedSegmentDataValues() {
	//convert enabled
	...
	//form unselected
	setDifference< SEGMENT_DATA_TYPE >( mEnabledSegmentDataValues, mSelectedSegmentDataValues, mUnselectedSegmentDataValues);
...
const SegmentDataSet&
OmSegmentManager::GetEnabledSegmentDataValues() {
	if(mCachedSegmentDataValuesDirty) RefreshCachedSegmentDataValues();
	return mEnabledSegmentDataValues;
}


const SegmentDataSet&
OmSegmentManager::GetSelectedSegmentDataValues() {
	if(mCachedSegmentDataValuesDirty) RefreshCachedSegmentDataValues();	
...
const SegmentDataSet& 
OmSegmentManager::GetUnselectedSegmentDataValues() {
	if(mCachedSegmentDataValuesDirty) RefreshCachedSegmentDataValues();
...
const string&
OmSegmentManager::GetDirectoryPath() {
	return mDirectoryPath;
}

void 
OmSegmentManager::SetDirectoryPath(const string &dpath) {
