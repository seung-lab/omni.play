 ...
 */
Vector4<int>
OmCamera::GetViewport() const {
	return mViewport;
}


void 
OmCamera::SetViewport(const Vector4<int> &view) {
	mViewport = view;
	ApplyViewport();
}

void 
OmCamera::ApplyViewport() {
 ...
 */
Vector4<float>
OmCamera::GetPerspective() const {
	return mPerspective;
}

void
OmCamera::SetPerspective(const Vector4<int> &persp) {
	mPerspective = persp;
	ApplyPerspective();
}
 ...
 */
void 
OmCamera::ApplyReshape(const Vector2<int> &dims) {
	//update viewport
...
void
OmCamera::SetFocus(const Vector3f &focus) {
...
Quaternionf 
OmCamera::GetOrbitRotation() {
	return mOrbitRotation;
}

void 
OmCamera::SetOrbitRotation(const Quaternionf &rRotation) {
...
Quaternionf 
OmCamera::GetLookAtRotation() {
	return mLookAtRotation;
}

void 
OmCamera::SetLookAtRotation(const Quaternionf &rRotation) {
...
const Matrix4<float>& 
OmCamera::GetModelViewMatrix() const {
	return mModelViewMatrix;
}


const Matrix4<float>& 
OmCamera::GetProjectionMatrix() const {
	return mProjectionMatrix;
}

Matrix4<float>
OmCamera::GetProjModelViewMatrix() const {
...
void
OmCamera::MovementStart(CameraMovementType type, const Vector2<float> &point) {
...
void
OmCamera::MovementUpdate(const Vector2<float> &point) {
...
void
OmCamera::MovementEnd(const Vector2<float> &point) {
