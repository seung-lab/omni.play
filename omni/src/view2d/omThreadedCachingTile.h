...
class OmThreadedCachingTile:public OmTile, public TextureIDThreadedCache {
 ...
 public:
	OmThreadedCachingTile(ViewType viewtype, ObjectType voltype, OmId image_id, OmMipVolume * vol,
			      const QGLContext * shareContext);

	~OmThreadedCachingTile();

	// texture ID
	virtual void GetTextureID(shared_ptr < OmTextureID > &p_value, const OmTileCoord & tileCoord, bool block =
				  true);
	void GetTextureIDDownMip(shared_ptr < OmTextureID > &p_value, const OmTileCoord & tileCoord, int rootLevel,
...
class OmCachingThreadedCachingTile {
 public:
	OmCachingThreadedCachingTile(ViewType viewtype, ObjectType voltype, OmId image_id, OmMipVolume * vol,
				     const QGLContext * shareContext) {
		...
		for (int i = 0; i < caches.size(); i++) {
			if (caches[i]->mViewtype == mViewtype &&
			    ...
			    caches[i]->mImage_id == mImage_id && caches[i]->mVol == mVol) {
