...
void 
OmCameraArcBall::Click(const Vector2<float> &point) {
...
void
OmCameraArcBall::Drag(const Vector2<float> &point) {	
	//find end vector
	...
	if( perp_vector.length() > EPSILON ) {	//if non-zero
		...
		rotation = Quaternion<float>( Vector3<float>::dot(mStartVector, mEndVector), perp_vector) * mStartRotation;
		rotation.normalize();
 ...
 */
void 
OmCameraArcBall::MapPointToSphere(const Vector2<float> &point, Vector3<float> &vector) {
...
void 
OmCameraPan::Click(const Vector2<float> &point) {
...
void 
OmCameraPan::Drag(const Vector2<float> &point) {
	...
	double d = double(r_viewport[3])/2.0 / tan(r_perspective[0]*M_PI / 180.0 / 2.0);
	...
	//update center
	center = mStartCenter +  orbit_mat.getRow(0) * move.x +  orbit_mat.getRow(1) * move.y;
...
void 
OmCameraZoom::Click(const Vector2<float> &point) {
...
void 
OmCameraZoom::Drag(const Vector2<float> &point)
{
