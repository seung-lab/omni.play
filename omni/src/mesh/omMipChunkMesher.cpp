...
OmMipChunkMesher *OmMipChunkMesher::Instance()
{
	if (NULL == mspInstance) {
...
void OmMipChunkMesher::BuildChunkMeshes(OmMipMeshManager * pMipMeshManager, shared_ptr < OmMipChunk > chunk,
					const SegmentDataSet & rMeshVals)
{
	debug("mesher", "OmMipChunkMesher::BuildChunkMeshes: (x,y,z):%i,%i,%i\n",
	      chunk->GetCoordinate().get < 1 > (),
	      chunk->GetCoordinate().get < 2 > (), chunk->GetCoordinate().get < 3 > ());

	Instance()->BuildChunkMeshesThreaded(pMipMeshManager, chunk, rMeshVals);

}

void OmMipChunkMesher::BuildChunkMeshesThreaded(OmMipMeshManager * pMipMeshManager, shared_ptr < OmMipChunk > chunk,
						const SegmentDataSet & rMeshVals)
{
 ...
 */
SEGMENT_DATA_TYPE OmMipChunkMesher::GetNextSegmentValueToMesh()
{
...
void OmMipChunkMesher::BuildMeshesLoop()
{
