 ...
 */
 template < class T > OmGenericManager < T >::OmGenericManager():mNextId(1)
{
 ...
 */
template < class T > OmGenericManager < T >::~OmGenericManager()
{
 ...
 */
template < class T > T & OmGenericManager < T >::Get(OmId omId)
{
 ...
 */
template < class T > T & OmGenericManager < T >::Add(OmId omId)
{
	//if no id specified
	if (NULL_OM_ID == omId) {
 ...
 */
template < class T > T & OmGenericManager < T >::Set(OmId omId, T * pObject)
{
	//if no id specified
	if (NULL_OM_ID == omId) {
 ...
 */
template < class T > void OmGenericManager < T >::Remove(OmId omId)
{
	//check volume with id exists
	if (mMap.count(omId) == 0) {
 ...
 */
template < class T > uint32_t OmGenericManager < T >::Size()
{
	return mMap.size();
}
 ...
 */
template < class T > bool OmGenericManager < T >::IsValid(OmId omId) const const
{
 ...
 */
template < class T > const OmIds & OmGenericManager < T >::GetValidIds() const const
{
	return mValidSet;
}

template < class T > void OmGenericManager < T >::CallValid(void (T::*fxn) ())
{
	Call(mValidSet, fxn);
}

template < class T > template < class U > void OmGenericManager < T >::CallValid(void (T::*fxn) (U), U arg)
{
 ...
 */
template < class T > bool OmGenericManager < T >::IsEnabled(OmId omId) const const
{
	if (!IsValid(omId))
 ...
 */
template < class T > void OmGenericManager < T >::SetEnabled(OmId omId, bool enable)
{
	if (!IsValid(omId))
		throw OmAccessException("Given id is not valid: " + omId);
 ...
 */
template < class T > void OmGenericManager < T >::SetAllEnabled(bool selected)
{
	if (selected)
 ...
 */
template < class T > const OmIds & OmGenericManager < T >::GetEnabledIds() const const
{
	return mEnabledSet;
}

template < class T > void OmGenericManager < T >::CallEnabled(void (T::*fxn) ())
{
	Call(mEnabledSet, fxn);
}

template < class T > template < class U > void OmGenericManager < T >::CallEnabled(void (T::*fxn) (U), U arg)
{
 ...
 */
template < class T > bool OmGenericManager < T >::IsSelected(OmId omId) const const
{
	if (!IsValid(omId))
		throw OmAccessException("Given id is not valid: " + omId);
 ...
 */
template < class T > void OmGenericManager < T >::SetSelected(OmId omId, bool selected)
{
 ...
 */
template < class T > void OmGenericManager < T >::SetAllSelected(bool selected)
{
	if (selected)
 ...
 */
template < class T > const OmIds & OmGenericManager < T >::GetSelectedIds() const const
{
	return mSelectedSet;
}

template < class T > void OmGenericManager < T >::CallSelected(void (T::*fxn) ())
{
	Call(mSelectedSet, fxn);
}

template < class T > template < class U > void OmGenericManager < T >::CallSelected(void (T::*fxn) (U), U arg)
{
 ...
 */
template < class T > void OmGenericManager < T >::Apply(void (*fxn) (T &))
{
	typename map < OmId, T * >::iterator it;
...
template < class T > void OmGenericManager < T >::Call(OmIds & set, void (T::*fxn) ())
{
...
template < class T > template < class U > void OmGenericManager < T >::Call(OmIds & set, void (T::*fxn) (U), U arg)
{
...
template < class T >
    ...
    inline void OmGenericManager < T >::serialize(Archive & ar, const unsigned int file_version)
{
	ar & mNextId;
	ar & mMap;
	ar & mValidSet;
