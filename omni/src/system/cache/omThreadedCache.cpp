...
template <typename KEY, typename PTR>
size_t OmThreadedCache<KEY,PTR>::Remove(const KEY &key)
{
...
template <typename KEY, typename PTR>
void OmThreadedCache<KEY,PTR>::RemoveOldest(const int64_t numBytesToRemove)
{
    ...
    if(entrySize_)
    {
        ...
        {
            numBytesRemoved += doRemoveOldest();
...
template <typename KEY, typename PTR>
int64_t OmThreadedCache<KEY,PTR>::doRemoveOldest()
{
    zi::rwmutex::write_guard g(mutex_);
    {
...
template <typename KEY, typename PTR>
void OmThreadedCache<KEY,PTR>::BlockingCreate(PTR& val, const KEY &key)
{
    val = HandleCacheMiss(key);
    cache_.set(key, val);
    keyAccessList_.touchPrefetch(key);
}
