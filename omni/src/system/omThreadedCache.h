 ...
 */
 ...
 template < typename T, typename U > class OmThreadedCache:public OmCacheBase {
 ...
 */
template < typename T, typename U > OmThreadedCache < T, U >::OmThreadedCache(OmCacheGroup group, bool initFetch)
 :OmCacheBase(group)
{
 ...
 */
template < typename T, typename U > OmThreadedCache < T, U >::~OmThreadedCache()
{
...
template < typename T, typename U >
    void OmThreadedCache < T, U >::Get(shared_ptr < U > &p_value, const T & key, bool blocking, bool exists)
{
	...
	if (mCachedValuesMap.count(key)) {
		...
		//if blocking cache
		if (OM_THREADED_CACHE_DEBUG)
			cout << "OmThreadedCache<T,U>::Get(): blocking fetch" << endl;
		...
		if ((0 == mFetchStack.count(key)) && (0 == mCurrentlyFetching.count(key))) {
			if (OM_THREADED_CACHE_DEBUG)
				cout << "OmThreadedCache<T,U>::Get(): threaded fetch" << endl;
 ...
 */
template < typename T, typename U > void OmThreadedCache < T, U >::Add(const T & key, U * value)
{
 ...
 */
template < typename T, typename U > void OmThreadedCache < T, U >::Remove(const T & key)
{
	//std::cerr << "OmThreadedCache::" << __FUNCTION__ << endl;
 ...
 */
template < typename T, typename U > bool OmThreadedCache < T, U >::RemoveOldest()
{
 ...
 */
template < typename T, typename U > bool OmThreadedCache < T, U >::Contains(const T & key)
{
	bool status;
 ...
 */
template < typename T, typename U > void OmThreadedCache < T, U >::Call(void (U::*fxn) (), bool lock)
{
 ...
 */
template < typename T, typename U > void OmThreadedCache < T, U >::Clear()
{
...
template < typename T, typename U > void OmThreadedCache < T, U >::SetFetchUpdateInterval(float interval)
{
	mFetchUpdateInterval = interval;
}

template < typename T, typename U > float OmThreadedCache < T, U >::GetFetchUpdateInterval()
{
	return mFetchUpdateInterval;
}

template < typename T, typename U > void OmThreadedCache < T, U >::SetFetchUpdateClearsFetchStack(bool state)
{
	mFetchUpdateClearsStack = state;
}

template < typename T, typename U > bool OmThreadedCache < T, U >::GetFetchUpdateClearsFetchStack()
{
...
template < typename T, typename U > bool OmThreadedCache < T, U >::IsFetchStackEmpty()
{
	bool empty;
	pthread_mutex_lock(&mCacheMutex);
 ...
 */
template < typename T, typename U > void OmThreadedCache < T, U >::FetchLoop()
{
	//alive and well
	...
	while (true) {
		...
		if (OM_THREADED_CACHE_DEBUG)
			cout << "OmThreadedCache<T,U>::FetchLoop(): sleeping " << endl;
		pthread_cond_wait(&mFetchThreadCv, &mFetchThreadMutex);
		if (OM_THREADED_CACHE_DEBUG)
			cout << "OmThreadedCache<T,U>::FetchLoop(): awake " << endl;
		...
		while (!IsFetchStackEmpty()) {
			...
			if (OM_THREADED_CACHE_DEBUG)
				cout << "OmThreadedCache<T,U>::FetchLoop(): lock mutex" << endl;
			pthread_mutex_lock(&mCacheMutex);
			...
			pthread_mutex_unlock(&mCacheMutex);
			if (OM_THREADED_CACHE_DEBUG)
				cout << "OmThreadedCache<T,U>::FetchLoop(): unlock mutex" << endl;
 ...
 */
template < typename T, typename U > bool OmThreadedCache < T, U >::FetchUpdateCheck()
{
...
template < typename T, typename U > bool OmThreadedCache < T, U >::InitializeFetchThread()
{
	return false;
}
 ...
 */
template < typename T, typename U > inline void *start_cache_fetch_thread(void *arg)
{
