...
//project
const string& 
OmStateManager::GetProjectFileName() {
	return Instance()->mProjectFileName;
}

void 
OmStateManager::SetProjectFileName(const string &name) {
	Instance()->mProjectFileName = name;
}

const string& 
OmStateManager::GetProjectDirectoryPath() {
	return Instance()->mProjectDirectoryPath;
}

void 
OmStateManager::SetProjectDirectoryPath(const string &dpath) {
...
Vector2<float> 
OmStateManager::GetViewSliceMin(ViewType plane) {
	switch(plane) {
		case XY_VIEW:
...
Vector2<float> 
OmStateManager::GetViewSliceMax(ViewType plane) {
	switch(plane) {
		case XY_VIEW:
...
float 
OmStateManager::GetViewSliceDepth(ViewType plane) {
	switch(plane) {
		case XY_VIEW:
 ...
 */
void
OmStateManager::SetZoomLevel(Vector2<int> zoom) {
 ...
 */
void
OmStateManager::SetPanDistance(ViewType plane, Vector2<int> pan, bool postEvent) {
	switch(plane) {
		case XY_VIEW:
...
Vector2<int> 
OmStateManager::GetPanDistance(ViewType plane) {
	switch(plane) {
		case XY_VIEW:
 ...
 */
void 
OmStateManager::SetSliceState(OmSlicePlane plane, bool enabled) {
	switch(plane) {
...
OmStateManager::SetViewSlice(const OmSlicePlane plane, const Vector3<int> &dim, unsigned char *p_data) {
	...
	switch(plane) {
		...
		case SLICE_XY_PLANE:
			...
			if(Instance()->mViewSliceDataXY) delete Instance()->mViewSliceDataXY;
			Instance()->mViewSliceDataXY = p_data_copy;
			break;
			
		case SLICE_YZ_PLANE:
			Instance()->mViewSliceDimYZ = dim;
			if(Instance()->mViewSliceDataYZ) delete Instance()->mViewSliceDataYZ;
			Instance()->mViewSliceDataYZ = p_data_copy;
			break;
			
		case SLICE_XZ_PLANE:
			Instance()->mViewSliceDimXZ = dim;
			if(Instance()->mViewSliceDataXZ) delete Instance()->mViewSliceDataXZ;
...
OmSystemMode
OmStateManager::GetSystemMode() {
	return Instance()->mSystemMode;
}


void 
OmStateManager::SetSystemMode(const OmSystemMode new_mode) {
	
	//return if no change
	if(new_mode == Instance()->mSystemMode) return;
...
OmToolMode
OmStateManager::GetToolMode() {
	return Instance()->mToolMode;
}


void 
OmStateManager::SetToolMode(const OmToolMode new_mode) {	
	//return if no change
	if(new_mode == Instance()->mToolMode) return;
...
void 
OmStateManager::PushUndoCommand(QUndoCommand *cmd) {
 ...
 */
const QGLWidget* 
OmStateManager::GetPrimaryView3dWidget() {
 ...
 */
QGLContext* 
OmStateManager::GetSharedView3dContext() {
 ...
 */
void 
OmStateManager::MakeContextCurrent(QGLContext* pContext) {
 ...
 */
QGLContext* 
OmStateManager::GetSharedView2dContext(const QGLContext *pContext) {
...
float 
OmStateManager::GetTransparencyAlpha() {
	return Instance()->TRANSPARANCY_ALPHA;
}

void 
OmStateManager::SetTransparencyAlpha(float alpha) {
