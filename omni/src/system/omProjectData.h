...
class OmProjectData {
 ...
 public:
	...
	//image data io
	static void CreateImageData(string & path, Vector3 < int >dataDims, Vector3 < int >chunkDims,
				    int bytesPerSample);
...
template < class T > void OmProjectData::ArchiveWrite(const string & path, T * t)
{
	assert(IsOpen());
