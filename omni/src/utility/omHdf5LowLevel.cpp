...
void om_hdf5_file_create(string fpath)
{
	...
	hid_t file_id = H5Fcreate(fpath.c_str(), H5F_ACC_EXCL, H5P_DEFAULT, H5P_DEFAULT);
...
hid_t om_hdf5_file_open_with_lock(string fpath)
{
	...
	hid_t file_id;
	file_id = H5Fopen(fpath.c_str(), H5F_ACC_RDWR, H5P_DEFAULT);
	
	if (file_id < 0) {
		std::cerr << "\nCould not open HDF5 file \"" << fpath << "\"" << endl;
		std::cerr  << "exception at: " << __FUNCTION__ << "  " << __LINE__ << endl << endl; 
...
void om_hdf5_file_close(const char * fileName)
{
	assert (0);
}
...
void
om_hdf5_dataset_image_create_tree_overwrite(string fileName, const char *name, Vector3 < int >dataDims,
...
void
om_hdf5_dataset_image_write(string fileName, const char *name, DataBbox extent, int bytesPerSample,
			    vtkImageData * imageData)
{
HDF5LOCK();
...
void
om_hdf5_dataset_image_write_trim(string fileName, const char *name, DataBbox dataExtent, int bytesPerSample,
