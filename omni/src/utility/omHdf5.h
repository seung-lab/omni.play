...
void om_hdf5_dataset_image_create(hid_t fileId, const char *name, Vector3 < int >dataDims, Vector3 < int >chunkDims,
				  int bytesPerSample, bool unlimited = false);
void om_hdf5_dataset_image_create_tree_overwrite(hid_t fileId, const char *name, Vector3 < int >dataDims,
...
void om_hdf5_dataset_image_write(hid_t fileId, const char *name, DataBbox extent, int bytesPerSample,
				 vtkImageData * data);
void om_hdf5_dataset_image_write_trim(hid_t fileId, const char *name, DataBbox dataExtent, int bytesPerSample,
...
template < class T > inline void om_hdf5_archive_read(hid_t fileId, const char *name, T * t)
{
	//assert data exists
 ...
 */
template < class T > inline void om_hdf5_archive_write(hid_t fileId, const char *name, T * t)
{
