 ...
 */
ImageType
om_imagedata_parse_image_type(const string &fname) {
 ...
 */
vtkImageReader2* 
om_imagedata_get_reader(ImageType type) {
...
vtkImageReader2* 
om_imagedata_get_reader(const string &fname) {
 ...
 */
vtkImageWriter* 
om_imagedata_get_writer(ImageType type) {
	//switch for extention type
	switch(type) {
...
vtkImageWriter* 
om_imagedata_get_writer(string &fname) {
...
vtkImageData* 
om_imagedata_read(string dpath, list<string> &fnames, const DataBbox srcExtentBbox, const DataBbox dataExtentBbox, int bytesPerSample) {
...
vtkImageData* 
om_imagedata_read_hdf5(string dpath, list<string> &fnames, const DataBbox srcExtentBbox, const DataBbox dataExtentBbox, int bytesPerSample) {
	...
	//write image data
	vtkImageData* data = om_hdf5_dataset_image_read_trim(file_id, HDF5_DEFAULT_DATASET_NAME, dataExtentBbox, bytesPerSample);
	//close file
...
void 
om_imagedata_write(vtkImageData *data, string dpath, string fpattern, const DataBbox dataExtentBbox, int bytesPerSample) {
 ...
 */
void 
om_imagedata_write_hdf5(vtkImageData *data, string dpath, string fpattern, const DataBbox dataExtentBbox, int bytesPerSample) {
	om_imagedata_write_hdf5(data, dpath, fpattern, dataExtentBbox, dataExtentBbox, bytesPerSample);
}

void 
om_imagedata_write_hdf5(vtkImageData *data, string dpath, string fpattern, const DataBbox dstExtentBbox, const DataBbox dataExtentBbox, int bytesPerSample) {
	...
	if(!bfs::exists(bfs::path(fpath))) {
		...
		om_hdf5_dataset_image_create_tree_overwrite(file_id, HDF5_DEFAULT_DATASET_NAME, dest_dims, dest_dims, bytesPerSample, true);
...
Vector3<int> 
om_imagedata_get_dims(string dpath, const list<string> &fnames) {
...
Vector3<int> 
om_imagedata_get_dims_vtk(string dpath, const list<string> &fnames) {
...
Vector3<int> 
om_imagedata_get_dims_hdf5(string dpath, const list<string> &fnames) {
...
void
getVtkExtentFromAxisAlignedBoundingBox(const AxisAlignedBoundingBox<int>& aabb, int extent[]) {
...
void
setAxisAlignedBoundingBoxFromVtkExtent(const int extent[], AxisAlignedBoundingBox<int>& aabb) {
...
void
printImageData(vtkImageData *data) {
	int extent[6];
...
void
clearImageData(vtkImageData *data) {
	int dims[3];
...
vtkImageData*
allocImageData(Vector3<int> dims, int bytesPerSample) {
	//alloc data
	vtkImageData *data = vtkImageData::New();
...
vtkImageData*
createBlankImageData(Vector3<int> dims, int bytesPerSample, char value) {
	//alloc data
 ...
 */
void*
copyImageData(vtkImageData *srcData, const DataBbox &srcCopyBbox) {
	...
	assert((src_copy_extent[1] <= src_data_extent[1]) && 
		   (src_copy_extent[3] <= src_data_extent[3]) && 
		   (src_copy_extent[5] <= src_data_extent[5]));	
...
copyImageData(vtkImageData *dstData, const DataBbox &dstCopyBbox,
	...
	assert((dst_copy_extent[1] <= dst_data_extent[1]) && 
		   (dst_copy_extent[3] <= dst_data_extent[3]) && 
		   (dst_copy_extent[5] <= dst_data_extent[5]));
...
void
copyIntersectedImageDataFromOffset(vtkImageData *dstData, 
	...
	if(dst_intersection.isEmpty()) return;
...
void
appendImageDataPairs(vtkImageData **inputImageData, vtkImageData **outputImageData, int num_pairs, int axis) {
 ...
 */	
void 
om_imagedata_regex_match_dir_contents(string dpath, string regexStr, list<string> &rMatchFnames) {
...
bool
string_natural_comparison(const string& lhs, const string& rhs) {
 ...
 */
void 
om_imagedata_regex_match_dir_contents_sorted(string dpath, string regexStr, list<string> &rMatchFnames) {
 ...
 */
void 
om_imagedata_create_symlink_dir(string symlinkDpath, string srcDpath, list<string> &rSrcFnames) {
...
void 
om_imagedata_remove_symlink_dir(string symlink_dpath) {
 ...
 */
void 
makeFilePath(string &fpath, const string &dpath, const string &fpattern, int index) {
...
int 
countMatchesInDirectory(string &dpath, string &fpattern) {
