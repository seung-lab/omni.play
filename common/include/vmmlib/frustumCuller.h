...
public:
    ...
    Visibility testSphere( const Vector4< T >& sphere ) const;
	Visibility testAabb( const AxisAlignedBoundingBox< T >& aabb ) const;
	
	//temporary until camera class
	const Vector4< T > * getPlanes() const { return &_leftPlane; }
	const Vector4< T > & getNearPlane() const { return _nearPlane; }
 ...
 */
	...
	template < class T > 
	Visibility FrustumCuller< T >::testSphere( const Vector4<T>& sphere ) const
	{
	...
	/*
	 *	AxisAlignedBoundingBox Frustum Culling using positive/negative verticies
	 *	http://www.lighthouse3d.com/opengl/viewfrustum/index.php?gatest3
	 */
	template < class T > 
	Visibility FrustumCuller< T >::testAabb( const AxisAlignedBoundingBox< T >& aabb ) const {

		Visibility visibility = VISIBILITY_FULL;
		
		//for each plane
		const Vector4< T > *planes = &_leftPlane;		//dirty trick to treat 6 planes as single array
		for(int i=0; i<6; i++) {
			
			//check if pos vertex is outside of plane
			if( planes[i].getDistanceToPlane( aabb.getPosVertex( planes[i] ) ) < 0 )
				return VISIBILITY_NONE;
			
			//check if neg vertex is outside of plane
			else if( planes[i].getDistanceToPlane( aabb.getNegVertex( planes[i] ) ) < 0 )
				visibility = VISIBILITY_PARTIAL;
		}
