...
data::data(data::base_t v, const volumeSystem* vol, int mipLevel)
    : base_t(v), vol_(vol), mipLevel_(mipLevel) {
...
norm data::toNorm() const { return toGlobal().toNorm(vol_); }
...
dataBbox::dataBbox(data min, data max)
    : base_t(min, max), vol_(min.volume()), mipLevel_(min.level()) {
