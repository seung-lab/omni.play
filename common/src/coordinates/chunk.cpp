...
Chunk::Chunk(int level, int x, int y, int z)
    : base_t(Vector3i(x, y, z)), mipLevel_(level) {
  if (x < 0 || y < 0 || z < 0) {
    throw ArgException("Bad Chunk Coord");
  }
}
...
uint64_t Chunk::PtrOffset(const VolumeSystem &vol,
...
bool Chunk::operator!=(const Chunk &rhs) const { return !(*this == rhs); }
