...
chunk::chunk() : Level(-1), Coordinate(Vector3i(-1, -1, -1)) {}
...
std::string chunk::getCoordsAsString() const {
  std::stringstream ss;
  ss << Level << ":" << Coordinate.x << "," << Coordinate.y << ","
     << Coordinate.z;
  return ss.str();
}
...
uint64_t chunk::chunkPtrOffset(const volumeSystem *vol,
                               int64_t bytesPerVoxel) const {
  ...
  const int64_t chunkSize =
      chunkDims.x * chunkDims.y * chunkDims.z * bytesPerVoxel;
  ...
  ZiLOG(DEBUG, io) << "offset is: " << offset << " (" << volDims << ") for "
                   << Coordinate << "\n";
  return offset;
}

int chunk::sliceDepth(const volumeSystem *vol, global c,
                      common::viewType view) const {
  ...
  switch (view) {
    case common::XY_VIEW:
      return d.z - bounds.getMin().z;
    case common::XZ_VIEW:
      return d.y - bounds.getMin().y;
    case common::ZY_VIEW:
      return d.x - bounds.getMin().x;
  }
...
std::ostream &operator<<(std::ostream &out, const chunk &c) {
  out << "[" << c.Level;
  out << " (" << c.Coordinate.x << ", " << c.Coordinate.y << ", "
      << c.Coordinate.z << ")]";
  return out;
}
