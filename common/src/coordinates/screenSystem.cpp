...
ScreenSystem::ScreenSystem(om::common::ViewType viewType, int width, int height,
                           double scale, Global location)
    : viewType_(viewType) {
...
void ScreenSystem::UpdateTransformationMatrices() {
  ...
  const Vector3f pan = makeViewTypeVector3(totalViewport_.width / 2.0f,
                                           totalViewport_.height / 2.0f, 0.0f);

  // g to s scaling and translation
  globalToScreenMat_.setTranslation(
      makeViewTypeVector3(pan / zoomScale_ - location_));
